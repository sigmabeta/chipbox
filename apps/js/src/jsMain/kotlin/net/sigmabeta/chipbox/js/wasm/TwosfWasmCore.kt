...
class TwosfWasmCore(private val module: ChipboxTwosfModule) {
    ...
    fun loadTrack(path: String) {
        ...
        try {
            ...
            val rc = module._chipbox_twosf_load_file(ptr)
            if (rc != 0) error("vio2sf: ${lastError()}")
        } finally {
            module._free(ptr)
        }
    }
    ...
    fun play(frames: Int): ShortArray? {
        ...
        try {
            val produced = module._chipbox_twosf_play(ptr, frames)
            if (produced == 0) {
                val err = lastError()
                if (err.isNotBlank()) error("vio2sf: $err")
                return null
            }
    ...
    fun sampleRate(): Int = module._chipbox_twosf_sample_rate()

    fun teardown() = module._chipbox_twosf_teardown()

    private fun lastError(): String = module.UTF8ToString(module._chipbox_twosf_last_error())
}
