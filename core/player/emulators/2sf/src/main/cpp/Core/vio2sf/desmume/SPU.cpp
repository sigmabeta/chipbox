...
static  void adjust_channel_timer(channel_struct *chan)
{
...
//////////////////////////////////////////////////////////////////////////////
static  void Fetch8BitDataInternal(channel_struct *chan, s32 *data)
{
	u32 loc = u32floor_double(chan->sampcnt);
	*data = (s32)chan->buf8[loc] << 8;
}

static  void Fetch16BitDataInternal(const channel_struct * const chan, s32 *data)
{
	const s16* const buf16 = chan->buf16;
	*data = (s32)buf16[u32floor_double(chan->sampcnt)];
}

static  void FetchADPCMDataInternal(channel_struct * const chan, s32 * const data)
{
	// No sense decoding, just return the last sample
	if (chan->lastsampcnt != u32floor_double(chan->sampcnt)){

	    const u32 endExclusive = u32floor_double(chan->sampcnt + 1);
	    ...
	    chan->lastsampcnt = u32floor_double(chan->sampcnt);
    }

	*data = (s32)chan->pcm16b;
}

static  void FetchPSGDataInternal(channel_struct *chan, s32 *data)
{
	if(chan->num < 8)
	{
		*data = 0;
	...
	{
		*data = (s32)wavedutytbl[chan->waveduty][(u32floor_double(chan->sampcnt)) & 0x7];
	}
	else
	{
		if(chan->lastsampcnt == u32floor_double(chan->sampcnt))
		...
		u32 max = u32floor_double(chan->sampcnt);
...
static  void MixL(SPU_struct* SPU, channel_struct *chan, s32 data)
{
	data = spumuldiv7(data, chan->vol) >> chan->datashift;
	SPU->sndbuf[SPU->bufpos<<1] += data;
}

static  void MixR(SPU_struct* SPU, channel_struct *chan, s32 data)
{
	data = spumuldiv7(data, chan->vol) >> chan->datashift;
	SPU->sndbuf[(SPU->bufpos<<1)+1] += data;
}

static  void MixLR(SPU_struct* SPU, channel_struct *chan, s32 data)
{
	data = spumuldiv7(data, chan->vol) >> chan->datashift;
...
static  void TestForLoop(NDS_state *state, int FORMAT, SPU_struct *SPU, channel_struct *chan)
{
	const int shift = (FORMAT == 0 ? 2 : 1);
...
static  void TestForLoop2(NDS_state *state, SPU_struct *SPU, channel_struct *chan)
{
	chan->sampcnt += chan->sampinc;
...
static  void Fetch8BitData(SPUInterpolationMode INTERPOLATE_MODE, NDS_state *state, SPU_struct* const SPU, channel_struct *chan, s32 *data)
{
...
static  void Fetch16BitData(SPUInterpolationMode INTERPOLATE_MODE, NDS_state *state, SPU_struct* const SPU, channel_struct *chan, s32 *data)
{
	double saved_inc = chan->sampinc;
	chan->sampinc = 1.0;
...
static  void FetchADPCMData(SPUInterpolationMode INTERPOLATE_MODE, NDS_state *state, SPU_struct* const SPU, channel_struct *chan, s32 *data)
{
	double saved_inc = chan->sampinc;
	chan->sampinc = 1.0;
...
static  void FetchPSGData(SPUInterpolationMode INTERPOLATE_MODE, channel_struct *chan, s32 *data)
{
 ...
 static void SPU_Mix(int CHANNELS, SPU_struct* SPU, channel_struct *chan, s32 data)
{
	switch(CHANNELS)
	{
 ...
 static void ____SPU_ChanUpdate(NDS_state *state, int CHANNELS, int FORMAT, SPUInterpolationMode INTERPOLATE_MODE, SPU_struct* const SPU, channel_struct* const chan)
{
 ...
 static void ___SPU_ChanUpdate(NDS_state *state, int FORMAT, SPUInterpolationMode INTERPOLATE_MODE, const bool actuallyMix, SPU_struct* const SPU, channel_struct* const chan)
{
	if(!actuallyMix)
 ...
 static void __SPU_ChanUpdate(NDS_state *state, SPUInterpolationMode INTERPOLATE_MODE, const bool actuallyMix, SPU_struct* const SPU, channel_struct* const chan)
{
	___SPU_ChanUpdate(state,chan->format,INTERPOLATE_MODE,actuallyMix, SPU, chan);
}

 static void _SPU_ChanUpdate(NDS_state *state, const bool actuallyMix, SPU_struct* const SPU, channel_struct* const chan)
{
