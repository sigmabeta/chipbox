...
void loadFile(const char *filename_c_str) {
    ...
    unsigned int sample_rate =  32768;
    ...
    mCoreConfigSetIntValue(&core->config, "gba.audioHle", 0);
...
int32_t generateBuffer(int16_t *target_array, int32_t buffer_size_frames) {
    ...
    int32_t buffer_size_samples = buffer_size_frames * 2;

    if (m_output.buffer_size_samples != buffer_size_samples) {
        delete m_output.samples;
        m_core->setAudioBufferSize(m_core, buffer_size_samples);

        m_output.buffer_size_samples = buffer_size_samples;
        m_output.samples = static_cast<int16_t *>(malloc(buffer_size_samples * 2));
    }

    m_output.samples_available = 0;
    while (m_output.samples_available < buffer_size_samples) {
        m_core->runFrame(m_core);
    }
    samples_written = m_output.samples_available;

    memcpy(target_array, m_output.samples, buffer_size_samples * 2);
...
static void _gsf_postAudioBuffer(struct mAVStream * stream, blip_t * left, blip_t * right)
{
    ...
    blip_read_samples(left, state->samples, m_output.buffer_size_samples, true);
    blip_read_samples(right, state->samples + 1, m_output.buffer_size_samples, true);
