...
#ifndef DISABLE_SSF
  struct SEGA_STATE *pState = (struct SEGA_STATE *) (state);
  uint32_t offset = pState->offset_to_satsound;
  if((offset != 0)) {
...
static void *getyamstate(struct SEGA_STATE *state) {
  void *yamstate = NULL;
#ifndef DISABLE_SSF
  uint32_t offset = ((struct SEGA_STATE *) (state))->offset_to_satsound;
  if((offset != 0)) { yamstate = satsound_get_yam_state(SATSOUNDSTATE); }
#endif
