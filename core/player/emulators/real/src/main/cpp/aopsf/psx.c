...
static const UINT8 mips_reg_layout[] =
        {
                MIPS_PC, -1,
...
static const UINT8 mips_win_layout[] = {
...
void logerror(MIPS_CPU_CONTEXT *mipscpu, const char *a, ...) {
    va_list vl;
    va_start(vl, a);
    mipscpu->psx->error_ptr += vsprintf(mipscpu->psx->error_ptr, a, vl);
    va_end(vl);
}
...
static UINT32 getcp2dr(MIPS_CPU_CONTEXT *mipscpu, int n_reg);

static void setcp2dr(MIPS_CPU_CONTEXT *mipscpu, int n_reg, UINT32 n_value);

static UINT32 getcp2cr(MIPS_CPU_CONTEXT *mipscpu, int n_reg);

static void setcp2cr(MIPS_CPU_CONTEXT *mipscpu, int n_reg, UINT32 n_value);
...
INLINE static void mips_set_cp0r(MIPS_CPU_CONTEXT *mipscpu, int reg, UINT32 value) {
    mipscpu->cp0r[reg] = value;
    if (reg == CP0_SR || reg == CP0_CAUSE) {
        if ((mipscpu->cp0r[CP0_SR] & SR_IEC) != 0 &&
            (mipscpu->cp0r[CP0_SR] & mipscpu->cp0r[CP0_CAUSE] & CAUSE_IP) != 0) {
            mips_exception(mipscpu, EXC_INT);
        } else if (mipscpu->delayr != REGPC &&
                   (mipscpu->pc & (((mipscpu->cp0r[CP0_SR] & SR_KUC) << 30) | 3)) != 0) {
            mips_exception(mipscpu, EXC_ADEL);
            mips_set_cp0r(mipscpu, CP0_BADVADDR, mipscpu->pc);
        }
    }
}

INLINE void mips_commit_delayed_load(MIPS_CPU_CONTEXT *mipscpu) {
    if (mipscpu->delayr != 0) {
...
INLINE void mips_delayed_branch(MIPS_CPU_CONTEXT *mipscpu, UINT32 n_adr) {
    if ((n_adr & (((mipscpu->cp0r[CP0_SR] & SR_KUC) << 30) | 3)) != 0) {
        mips_exception(mipscpu, EXC_ADEL);
        mips_set_cp0r(mipscpu, CP0_BADVADDR, n_adr);
    } else {
        mips_commit_delayed_load(mipscpu);
...
INLINE void mips_set_pc(MIPS_CPU_CONTEXT *mipscpu, unsigned val) {
    mipscpu->pc = val;
    change_pc(val);
    mipscpu->delayr = 0;
    mipscpu->delayv = 0;
...
INLINE void mips_advance_pc(MIPS_CPU_CONTEXT *mipscpu) {
    if (mipscpu->delayr == REGPC) {
        mips_set_pc(mipscpu, mipscpu->delayv);
    } else {
        mips_commit_delayed_load(mipscpu);
        mipscpu->pc += 4;
    }
...
INLINE void mips_load(MIPS_CPU_CONTEXT *mipscpu, UINT32 n_r, UINT32 n_v) {
...
INLINE void mips_delayed_load(MIPS_CPU_CONTEXT *mipscpu, UINT32 n_r, UINT32 n_v) {
    if (mipscpu->delayr == REGPC) {
        mips_set_pc(mipscpu, mipscpu->delayv);
        mipscpu->delayr = n_r;
        mipscpu->delayv = n_v;
...
static void mips_exception(MIPS_CPU_CONTEXT *mipscpu, int exception) {
    mips_set_cp0r(mipscpu, CP0_SR,
                  (mipscpu->cp0r[CP0_SR] & ~0x3f) | ((mipscpu->cp0r[CP0_SR] << 2) & 0x3f));
    if (mipscpu->delayr == REGPC) {
        mips_set_cp0r(mipscpu, CP0_EPC, mipscpu->pc - 4);
        mips_set_cp0r(mipscpu, CP0_CAUSE,
                      (mipscpu->cp0r[CP0_CAUSE] & ~CAUSE_EXC) | CAUSE_BD | (exception << 2));
    } else {
        mips_commit_delayed_load(mipscpu);
        mips_set_cp0r(mipscpu, CP0_EPC, mipscpu->pc);
        mips_set_cp0r(mipscpu, CP0_CAUSE,
                      (mipscpu->cp0r[CP0_CAUSE] & ~(CAUSE_EXC | CAUSE_BD)) | (exception << 2));
    }
    if (mipscpu->cp0r[CP0_SR] & SR_BEV) {
        mips_set_pc(mipscpu, 0xbfc00180);
    } else {
        mips_set_pc(mipscpu, 0x80000080);
    }
}

void mips_init(MIPS_CPU_CONTEXT *mipscpu) {
#if 0
...
void mips_reset(MIPS_CPU_CONTEXT *mipscpu, void *param) {
    mips_set_cp0r(mipscpu, CP0_SR,
                  (mipscpu->cp0r[CP0_SR] & ~(SR_TS | SR_SWC | SR_KUC | SR_IEC)) | SR_BEV);
    mips_set_cp0r(mipscpu, CP0_RANDOM, 63); /* todo: */
    mips_set_cp0r(mipscpu, CP0_PRID, 0x00000200); /* todo: */
    mips_set_pc(mipscpu, 0xbfc00000);
    mipscpu->prevpc = 0xffffffff;
}

static void mips_exit(MIPS_CPU_CONTEXT *mipscpu) {
}

void mips_shorten_frame(MIPS_CPU_CONTEXT *mipscpu) {
    mipscpu->mips_ICount = 0;
}

void psx_hw_runcounters(PSX_STATE *);

int mips_execute(MIPS_CPU_CONTEXT *mipscpu, int cycles) {
    UINT32 n_res;

    mipscpu->mips_ICount = cycles;
    do {
//		CALL_MAME_DEBUG;
...
//		psx_hw_runcounters();
        ...
        mipscpu->op = cpu_readop32(mipscpu->pc);
            ...
            if (mipscpu->op == FUNCT_HLECALL)
                strcpy(buffer, "hlecall");
            else
                r3000dis(buffer, 0, mipscpu->pc, mipscpu->op);
            fprintf(mipscpu->file, "%08x:\t%s\n", mipscpu->pc, buffer);
        }
#endif
#if 0
                                                                                                                                while (mipscpu->prevpc == mipscpu->pc)
		{
			psx_hw_runcounters(mipscpu->psx);
			mipscpu->mips_ICount--;

			if (mipscpu->mips_ICount == 0) return cycles;
		}
...
//				printf("HLECALL, PC = %08x\n", mipscpu->pc);
                        psx_bios_hle(mipscpu->psx, mipscpu->pc);
                        break;
                    case FUNCT_SLL:
                        mips_load(mipscpu, INS_RD(mipscpu->op),
                                  mipscpu->r[INS_RT(mipscpu->op)] << INS_SHAMT(mipscpu->op));
                        break;
                    case FUNCT_SRL:
                        mips_load(mipscpu, INS_RD(mipscpu->op),
                                  mipscpu->r[INS_RT(mipscpu->op)] >> INS_SHAMT(mipscpu->op));
                    ...
                    case FUNCT_SRA:
                        mips_load(mipscpu, INS_RD(mipscpu->op),
                                  (INT32) mipscpu->r[INS_RT(mipscpu->op)]
                                          >> INS_SHAMT(mipscpu->op));
                        break;
                    case FUNCT_SLLV:
                        mips_load(mipscpu, INS_RD(mipscpu->op), mipscpu->r[INS_RT(mipscpu->op)]
                                << (mipscpu->r[INS_RS(mipscpu->op)] & 31));
                        break;
                    case FUNCT_SRLV:
                        mips_load(mipscpu, INS_RD(mipscpu->op), mipscpu->r[INS_RT(mipscpu->op)]
                                >> (mipscpu->r[INS_RS(mipscpu->op)] & 31));
                    ...
                    case FUNCT_JR:
                        if (INS_RD(mipscpu->op) != 0) {
                            mips_exception(mipscpu, EXC_RI);
                        } else {
                            mips_delayed_branch(mipscpu, mipscpu->r[INS_RS(mipscpu->op)]);
                        }
                        break;
                    case FUNCT_JALR:
                        n_res = mipscpu->pc + 8;
                        ...
                        break;
                    case FUNCT_SYSCALL:
                        mips_exception(mipscpu, EXC_SYS);
                        break;
                    case FUNCT_BREAK:
                        logerror(mipscpu, "BREAK!\n");
                        mips_stop(mipscpu);
//				exit(-1);
//				mips_exception( EXC_BP );
                        ...
                        break;
                    case FUNCT_MFHI:
                        mips_load(mipscpu, INS_RD(mipscpu->op), mipscpu->hi);
                        break;
                    case FUNCT_MTHI:
                        if (INS_RD(mipscpu->op) != 0) {
                            mips_exception(mipscpu, EXC_RI);
                        ...
                        break;
                    case FUNCT_MFLO:
                        mips_load(mipscpu, INS_RD(mipscpu->op), mipscpu->lo);
                        break;
                    case FUNCT_MTLO:
                        if (INS_RD(mipscpu->op) != 0) {
                            mips_exception(mipscpu, EXC_RI);
                    ...
                    case FUNCT_MULT:
                        if (INS_RD(mipscpu->op) != 0) {
                            ...
                            INT64 n_res64;
                            n_res64 = MUL_64_32_32((INT32) mipscpu->r[INS_RS(mipscpu->op)],
                                                   (INT32) mipscpu->r[INS_RT(mipscpu->op)]);
                            mips_advance_pc(mipscpu);
                    ...
                    case FUNCT_MULTU:
                        if (INS_RD(mipscpu->op) != 0) {
                            ...
                            UINT64 n_res64;
                            n_res64 = MUL_U64_U32_U32(mipscpu->r[INS_RS(mipscpu->op)],
                                                      mipscpu->r[INS_RT(mipscpu->op)]);
                            mips_advance_pc(mipscpu);
                    ...
                    case FUNCT_DIV:
                        if (INS_RD(mipscpu->op) != 0) {
                            ...
                            if (mipscpu->r[INS_RT(mipscpu->op)] != 0) {
                                n_div = (INT32) mipscpu->r[INS_RS(mipscpu->op)] /
                                        (INT32) mipscpu->r[INS_RT(mipscpu->op)];
                                n_mod = (INT32) mipscpu->r[INS_RS(mipscpu->op)] %
                                        (INT32) mipscpu->r[INS_RT(mipscpu->op)];
                                mips_advance_pc(mipscpu);
                                mipscpu->lo = n_div;
                                mipscpu->hi = n_mod;
                    ...
                    case FUNCT_DIVU:
                        if (INS_RD(mipscpu->op) != 0) {
                            ...
                            UINT32 n_mod;
                            if (mipscpu->r[INS_RT(mipscpu->op)] != 0) {
                                n_div = mipscpu->r[INS_RS(mipscpu->op)] /
                                        mipscpu->r[INS_RT(mipscpu->op)];
                                n_mod = mipscpu->r[INS_RS(mipscpu->op)] %
                                        mipscpu->r[INS_RT(mipscpu->op)];
                                mips_advance_pc(mipscpu);
                                mipscpu->lo = n_div;
                                mipscpu->hi = n_mod;
                    ...
                    case FUNCT_ADD: {
                        ...
                        if ((INT32) (~(mipscpu->r[INS_RS(mipscpu->op)] ^
                                       mipscpu->r[INS_RT(mipscpu->op)]) &
                                     (mipscpu->r[INS_RS(mipscpu->op)] ^ n_res)) < 0) {
                            mips_exception(mipscpu, EXC_OVF);
                        } else {
                            mips_load(mipscpu, INS_RD(mipscpu->op), n_res);
                        }
                    }
                        break;
                    case FUNCT_ADDU:
                        mips_load(mipscpu, INS_RD(mipscpu->op), mipscpu->r[INS_RS(mipscpu->op)] +
                                                                mipscpu->r[INS_RT(mipscpu->op)]);
                        break;
                    case FUNCT_SUB:
                        n_res = mipscpu->r[INS_RS(mipscpu->op)] - mipscpu->r[INS_RT(mipscpu->op)];
                    ...
                    case FUNCT_SLT:
                        mips_load(mipscpu, INS_RD(mipscpu->op),
                                  (INT32) mipscpu->r[INS_RS(mipscpu->op)] <
                                  (INT32) mipscpu->r[INS_RT(mipscpu->op)]);
                        break;
                    case FUNCT_SLTU:
                        mips_load(mipscpu, INS_RD(mipscpu->op), mipscpu->r[INS_RS(mipscpu->op)] <
            ...
            case OP_REGIMM:
                switch (INS_RT(mipscpu->op)) {
                    case RT_BLTZ:
                        if ((INT32) mipscpu->r[INS_RS(mipscpu->op)] < 0) {
                            mips_delayed_branch(mipscpu, mipscpu->pc + 4 + (MIPS_WORD_EXTEND(
                                    INS_IMMEDIATE(mipscpu->op)) << 2));
                        } else {
                            mips_advance_pc(mipscpu);
                        }
                        break;
                ...
                break;
            case OP_J:
                mips_delayed_branch(mipscpu, ((mipscpu->pc + 4) & 0xf0000000) +
                                             (INS_TARGET(mipscpu->op) << 2));
                break;
            case OP_JAL:
                n_res = mipscpu->pc + 8;
                mips_delayed_branch(mipscpu, ((mipscpu->pc + 4) & 0xf0000000) +
            ...
            case OP_BEQ:
                if (mipscpu->r[INS_RS(mipscpu->op)] == mipscpu->r[INS_RT(mipscpu->op)]) {
                    mips_delayed_branch(mipscpu, mipscpu->pc + 4 +
                                                 (MIPS_WORD_EXTEND(INS_IMMEDIATE(mipscpu->op))
                                                         << 2));
                } else {
                    mips_advance_pc(mipscpu);
                }
            ...
            case OP_ADDI: {
                ...
                if ((INT32) (~(mipscpu->r[INS_RS(mipscpu->op)] ^ n_imm) &
                             (mipscpu->r[INS_RS(mipscpu->op)] ^ n_res)) < 0) {
                    mips_exception(mipscpu, EXC_OVF);
                } else {
                    mips_load(mipscpu, INS_RT(mipscpu->op), n_res);
                }
            }
                break;
            case OP_ADDIU:
                if (INS_RT(mipscpu->op) == 0) {
                    psx_iop_call(mipscpu->psx, mipscpu->pc, INS_IMMEDIATE(mipscpu->op));
                    mips_advance_pc(mipscpu);
                    if (!mipscpu->psx->irq_mutex && mipscpu->psx->rescheduleNeeded) {
                        ps2_reschedule(mipscpu->psx);
                    }
                } else {
                    mips_load(mipscpu, INS_RT(mipscpu->op), mipscpu->r[INS_RS(mipscpu->op)] +
                                                            MIPS_WORD_EXTEND(
                                                                    INS_IMMEDIATE(mipscpu->op)));
                }
                break;
            case OP_SLTI:
                mips_load(mipscpu, INS_RT(mipscpu->op), (INT32) mipscpu->r[INS_RS(mipscpu->op)] <
                                                        MIPS_WORD_EXTEND(
                                                                INS_IMMEDIATE(mipscpu->op)));
                break;
            case OP_SLTIU:
                mips_load(mipscpu, INS_RT(mipscpu->op), mipscpu->r[INS_RS(mipscpu->op)] <
                                                        (UINT32) MIPS_WORD_EXTEND(
                                                                INS_IMMEDIATE(mipscpu->op)));
                break;
            case OP_ANDI:
                mips_load(mipscpu, INS_RT(mipscpu->op),
                          mipscpu->r[INS_RS(mipscpu->op)] & INS_IMMEDIATE(mipscpu->op));
                break;
            case OP_ORI:
                mips_load(mipscpu, INS_RT(mipscpu->op),
                          mipscpu->r[INS_RS(mipscpu->op)] | INS_IMMEDIATE(mipscpu->op));
            ...
            case OP_XORI:
                ...
                break;
            case OP_LUI:
                mips_load(mipscpu, INS_RT(mipscpu->op), INS_IMMEDIATE(mipscpu->op) << 16);
                break;
            case OP_COP0:
                if ((mipscpu->cp0r[CP0_SR] & SR_KUC) != 0 &&
                    (mipscpu->cp0r[CP0_SR] & SR_CU0) == 0) {
                    mips_exception(mipscpu, EXC_CPU);
                    mips_set_cp0r(mipscpu, CP0_CAUSE,
                                  (mipscpu->cp0r[CP0_CAUSE] & ~CAUSE_CE) | CAUSE_CE0);
                } else {
                    switch (INS_RS(mipscpu->op)) {
                        case RS_MFC:
                            mips_delayed_load(mipscpu, INS_RT(mipscpu->op),
                                              mipscpu->cp0r[INS_RD(mipscpu->op)]);
                            break;
                        case RS_CFC:
                            /* todo: */
                            logerror(mipscpu, "%08x: COP0 CFC not supported\n", mipscpu->pc);
                            mips_stop(mipscpu);
                            mips_advance_pc(mipscpu);
                            break;
                        case RS_MTC:
                            n_res = (mipscpu->cp0r[INS_RD(mipscpu->op)] &
                                     ~mips_mtc0_writemask[INS_RD(mipscpu->op)]) |
                                    (mipscpu->r[INS_RT(mipscpu->op)] &
                                     mips_mtc0_writemask[INS_RD(mipscpu->op)]);
                            mips_advance_pc(mipscpu);
                            mips_set_cp0r(mipscpu, INS_RD(mipscpu->op), n_res);
                            break;
                        case RS_CTC:
                            /* todo: */
                            logerror(mipscpu, "%08x: COP0 CTC not supported\n", mipscpu->pc);
                            mips_stop(mipscpu);
                            mips_advance_pc(mipscpu);
                            break;
                        case RS_BC:
                            switch (INS_RT(mipscpu->op)) {
                                case RT_BCF:
                                    /* todo: */
                                    logerror(mipscpu, "%08x: COP0 BCF not supported\n",
                                             mipscpu->pc);
                                    mips_stop(mipscpu);
                                    mips_advance_pc(mipscpu);
                                    break;
                                case RT_BCT:
                                    /* todo: */
                                    logerror(mipscpu, "%08x: COP0 BCT not supported\n",
                                             mipscpu->pc);
                                    mips_stop(mipscpu);
                                    mips_advance_pc(mipscpu);
                                    break;
                                default:
                                    /* todo: */
                                    logerror(mipscpu, "%08x: COP0 unknown command %08x\n",
                                ...
                                case 1:
                                    switch (INS_CF(mipscpu->op)) {
                                        case CF_RFE:
                                            mips_advance_pc(mipscpu);
                                            mips_set_cp0r(mipscpu, CP0_SR,
                                                          (mipscpu->cp0r[CP0_SR] & ~0xf) |
                                                          ((mipscpu->cp0r[CP0_SR] >> 2) & 0xf));
                                            break;
                                        default:
                                            /* todo: */
                                            logerror(mipscpu, "%08x: COP0 unknown command %08x\n",
            ...
            case OP_COP1:
                if ((mipscpu->cp0r[CP0_SR] & SR_CU1) == 0) {
                    mips_exception(mipscpu, EXC_CPU);
                    mips_set_cp0r(mipscpu, CP0_CAUSE,
                                  (mipscpu->cp0r[CP0_CAUSE] & ~CAUSE_CE) | CAUSE_CE1);
                } else {
                    switch (INS_RS(mipscpu->op)) {
                        case RS_MFC:
                            /* todo: */
                            logerror(mipscpu, "%08x: COP1 BCT not supported\n", mipscpu->pc);
                            mips_stop(mipscpu);
                            mips_advance_pc(mipscpu);
                            break;
                        case RS_CFC:
                            /* todo: */
                            logerror(mipscpu, "%08x: COP1 CFC not supported\n", mipscpu->pc);
                            mips_stop(mipscpu);
                            mips_advance_pc(mipscpu);
                            break;
                        case RS_MTC:
                            /* todo: */
                            logerror(mipscpu, "%08x: COP1 MTC not supported\n", mipscpu->pc);
                            mips_stop(mipscpu);
                            mips_advance_pc(mipscpu);
                            break;
                        case RS_CTC:
                            /* todo: */
                            logerror(mipscpu, "%08x: COP1 CTC not supported\n", mipscpu->pc);
                            mips_stop(mipscpu);
                            mips_advance_pc(mipscpu);
                            break;
                        case RS_BC:
                            switch (INS_RT(mipscpu->op)) {
                                case RT_BCF:
                                    /* todo: */
                                    logerror(mipscpu, "%08x: COP1 BCF not supported\n",
                                             mipscpu->pc);
                                    mips_stop(mipscpu);
                                    mips_advance_pc(mipscpu);
                                    break;
                                ...
                                default:
                                    /* todo: */
                                    logerror(mipscpu, "%08x: COP1 unknown command %08x\n",
                                             mipscpu->pc, mipscpu->op);
                                    mips_stop(mipscpu);
                                    mips_advance_pc(mipscpu);
                                    break;
            ...
            case OP_COP2:
                if ((mipscpu->cp0r[CP0_SR] & SR_CU2) == 0) {
                    mips_exception(mipscpu, EXC_CPU);
                    mips_set_cp0r(mipscpu, CP0_CAUSE,
                                  (mipscpu->cp0r[CP0_CAUSE] & ~CAUSE_CE) | CAUSE_CE2);
                } else {
                    switch (INS_RS(mipscpu->op)) {
                        case RS_MFC:
                            mips_delayed_load(mipscpu, INS_RT(mipscpu->op),
                                              getcp2dr(mipscpu, INS_RD(mipscpu->op)));
                            break;
                        case RS_CFC:
                            mips_delayed_load(mipscpu, INS_RT(mipscpu->op),
                                              getcp2cr(mipscpu, INS_RD(mipscpu->op)));
                            break;
                        case RS_MTC:
                            setcp2dr(mipscpu, INS_RD(mipscpu->op), mipscpu->r[INS_RT(mipscpu->op)]);
                            mips_advance_pc(mipscpu);
                            break;
                        case RS_CTC:
                            setcp2cr(mipscpu, INS_RD(mipscpu->op), mipscpu->r[INS_RT(mipscpu->op)]);
                            mips_advance_pc(mipscpu);
                            break;
                        case RS_BC:
                            switch (INS_RT(mipscpu->op)) {
                                case RT_BCF:
                                    /* todo: */
                                    logerror(mipscpu, "%08x: COP2 BCF not supported\n",
                                             mipscpu->pc);
                                    mips_stop(mipscpu);
                                    mips_advance_pc(mipscpu);
                                    break;
                                case RT_BCT:
                                    /* todo: */
                                    logerror(mipscpu, "%08x: COP2 BCT not supported\n",
                                             mipscpu->pc);
                                    mips_stop(mipscpu);
                                    mips_advance_pc(mipscpu);
                                    break;
                                default:
                                    /* todo: */
                                    logerror(mipscpu, "%08x: COP2 unknown command %08x\n",
                                             mipscpu->pc, mipscpu->op);
                                    mips_stop(mipscpu);
                                    mips_advance_pc(mipscpu);
                                    break;
                        ...
                        default:
                            switch (INS_CO(mipscpu->op)) {
                                case 1:
                                    docop2(mipscpu, INS_COFUN(mipscpu->op));
                                    mips_advance_pc(mipscpu);
                                    break;
                                default:
                                    /* todo: */
            ...
            case OP_LB:
                if ((mipscpu->cp0r[CP0_SR] & SR_ISC) != 0) {
                    /* todo: */
                    logerror(mipscpu, "%08x: LB SR_ISC not supported\n", mipscpu->pc);
                    mips_stop(mipscpu);
                    mips_advance_pc(mipscpu);
                } else if ((mipscpu->cp0r[CP0_SR] & (SR_RE | SR_KUC)) == (SR_RE | SR_KUC)) {
                    UINT32 n_adr;
                    n_adr = mipscpu->r[INS_RS(mipscpu->op)] +
                            MIPS_WORD_EXTEND(INS_IMMEDIATE(mipscpu->op));
                    if ((n_adr & ((mipscpu->cp0r[CP0_SR] & SR_KUC) << 30)) != 0) {
                        mips_exception(mipscpu, EXC_ADEL);
                        mips_set_cp0r(mipscpu, CP0_BADVADDR, n_adr);
                    } else {
                        mips_delayed_load(mipscpu, INS_RT(mipscpu->op), MIPS_BYTE_EXTEND(
                                program_read_byte_32le(mipscpu->psx, n_adr ^ 3)));
                    }
                } else {
                    UINT32 n_adr;
                    n_adr = mipscpu->r[INS_RS(mipscpu->op)] +
                    ...
                    if ((n_adr & ((mipscpu->cp0r[CP0_SR] & SR_KUC) << 30)) != 0) {
                        ...
                        mips_delayed_load(mipscpu, INS_RT(mipscpu->op), MIPS_BYTE_EXTEND(
                                program_read_byte_32le(mipscpu->psx, n_adr)));
                    }
                }
                break;
            case OP_LH:
                if ((mipscpu->cp0r[CP0_SR] & SR_ISC) != 0) {
                    /* todo: */
                    logerror(mipscpu, "%08x: LH SR_ISC not supported\n", mipscpu->pc);
                    mips_stop(mipscpu);
                    mips_advance_pc(mipscpu);
                } else if ((mipscpu->cp0r[CP0_SR] & (SR_RE | SR_KUC)) == (SR_RE | SR_KUC)) {
                    UINT32 n_adr;
                    ...
                    if ((n_adr & (((mipscpu->cp0r[CP0_SR] & SR_KUC) << 30) | 1)) != 0) {
                        ...
                        mips_delayed_load(mipscpu, INS_RT(mipscpu->op), MIPS_WORD_EXTEND(
                                program_read_word_32le(mipscpu->psx, n_adr ^ 2)));
                    }
                } else {
                    UINT32 n_adr;
                    n_adr = mipscpu->r[INS_RS(mipscpu->op)] +
                    ...
                    if ((n_adr & (((mipscpu->cp0r[CP0_SR] & SR_KUC) << 30) | 1)) != 0) {
                        ...
                        mips_delayed_load(mipscpu, INS_RT(mipscpu->op), MIPS_WORD_EXTEND(
                                program_read_word_32le(mipscpu->psx, n_adr)));
                    }
                }
                break;
            case OP_LWL:
                if ((mipscpu->cp0r[CP0_SR] & SR_ISC) != 0) {
                    /* todo: */
                    logerror(mipscpu, "%08x: LWL SR_ISC not supported\n", mipscpu->pc);
                    mips_stop(mipscpu);
                    mips_advance_pc(mipscpu);
                } else if ((mipscpu->cp0r[CP0_SR] & (SR_RE | SR_KUC)) == (SR_RE | SR_KUC)) {
                    UINT32 n_adr;
                    ...
                    if ((n_adr & ((mipscpu->cp0r[CP0_SR] & SR_KUC) << 30)) != 0) {
                        ...
                        switch (n_adr & 3) {
                            case 0:
                                n_res = (mipscpu->r[INS_RT(mipscpu->op)] & 0x00ffffff) |
                                        ((UINT32) program_read_byte_32le(mipscpu->psx, n_adr + 3)
                                                << 24);
                                break;
                            case 1:
                                n_res = (mipscpu->r[INS_RT(mipscpu->op)] & 0x0000ffff) |
                                        ((UINT32) program_read_word_32le(mipscpu->psx, n_adr + 1)
                                                << 16);
                                break;
                            case 2:
                                n_res = (mipscpu->r[INS_RT(mipscpu->op)] & 0x000000ff) |
                                        ((UINT32) program_read_byte_32le(mipscpu->psx, n_adr - 1)
                                                << 8) |
                                        ((UINT32) program_read_word_32le(mipscpu->psx, n_adr)
                                                << 16);
                                break;
                            default:
                                n_res = program_read_dword_32le(mipscpu->psx, n_adr - 3);
                                break;
                        }
                        mips_delayed_load(mipscpu, INS_RT(mipscpu->op), n_res);
                    }
                } else {
                    UINT32 n_adr;
                    n_adr = mipscpu->r[INS_RS(mipscpu->op)] +
                    ...
                    if ((n_adr & ((mipscpu->cp0r[CP0_SR] & SR_KUC) << 30)) != 0) {
                        ...
                        switch (n_adr & 3) {
                            case 0:
                                n_res = (mipscpu->r[INS_RT(mipscpu->op)] & 0x00ffffff) |
                                        ((UINT32) program_read_byte_32le(mipscpu->psx, n_adr)
                                                << 24);
                                break;
                            case 1:
                                n_res = (mipscpu->r[INS_RT(mipscpu->op)] & 0x0000ffff) |
                            ...
                            case 2:
                                n_res = (mipscpu->r[INS_RT(mipscpu->op)] & 0x000000ff) |
                                        ((UINT32) program_read_word_32le(mipscpu->psx, n_adr - 2)
                                                << 8) |
                                        ((UINT32) program_read_byte_32le(mipscpu->psx, n_adr)
                                                << 24);
                                break;
            ...
            case OP_LW:
                if ((mipscpu->cp0r[CP0_SR] & SR_ISC) != 0) {
                    /* todo: */
                    logerror(mipscpu, "%08x: LW SR_ISC not supported\n", mipscpu->pc);
                    mips_stop(mipscpu);
                    mips_advance_pc(mipscpu);
                } else {
                    UINT32 n_adr;
                                                                                                                                            ...
                                                                                                                                            if( ( n_adr & ( ( ( mipscpu->cp0r[ CP0_SR ] & SR_KUC ) << 30 ) | 3 ) ) != 0 )
				{
                    ...
                    {
                        mips_delayed_load(mipscpu, INS_RT(mipscpu->op),
                                          program_read_dword_32le(mipscpu->psx, n_adr));
                    }
                }
                break;
            case OP_LBU:
                if ((mipscpu->cp0r[CP0_SR] & SR_ISC) != 0) {
                    /* todo: */
                    logerror(mipscpu, "%08x: LBU SR_ISC not supported\n", mipscpu->pc);
                    mips_stop(mipscpu);
                    mips_advance_pc(mipscpu);
                } else if ((mipscpu->cp0r[CP0_SR] & (SR_RE | SR_KUC)) == (SR_RE | SR_KUC)) {
                    UINT32 n_adr;
            ...
            case OP_LHU:
                if ((mipscpu->cp0r[CP0_SR] & SR_ISC) != 0) {
                    /* todo: */
                    logerror(mipscpu, "%08x: LHU SR_ISC not supported\n", mipscpu->pc);
                    mips_stop(mipscpu);
                    mips_advance_pc(mipscpu);
                } else if ((mipscpu->cp0r[CP0_SR] & (SR_RE | SR_KUC)) == (SR_RE | SR_KUC)) {
                    UINT32 n_adr;
            ...
            case OP_LWR:
                if ((mipscpu->cp0r[CP0_SR] & SR_ISC) != 0) {
                    /* todo: */
                    logerror(mipscpu, "%08x: LWR SR_ISC not supported\n", mipscpu->pc);
                    mips_stop(mipscpu);
                    mips_advance_pc(mipscpu);
                } else if ((mipscpu->cp0r[CP0_SR] & (SR_RE | SR_KUC)) == (SR_RE | SR_KUC)) {
                    UINT32 n_adr;
                    ...
                    if ((n_adr & ((mipscpu->cp0r[CP0_SR] & SR_KUC) << 30)) != 0) {
                        ...
                        switch (n_adr & 3) {
                            case 3:
                                n_res = (mipscpu->r[INS_RT(mipscpu->op)] & 0xffffff00) |
                                        program_read_byte_32le(mipscpu->psx, n_adr - 3);
                                break;
                            case 2:
                                n_res = (mipscpu->r[INS_RT(mipscpu->op)] & 0xffff0000) |
                                        program_read_word_32le(mipscpu->psx, n_adr - 2);
                                break;
                            case 1:
                                n_res = (mipscpu->r[INS_RT(mipscpu->op)] & 0xff000000) |
                                        program_read_word_32le(mipscpu->psx, n_adr - 1) |
                                        ((UINT32) program_read_byte_32le(mipscpu->psx, n_adr + 1)
                                                << 16);
                                break;
                            default:
                                n_res = program_read_dword_32le(mipscpu->psx, n_adr);
                                break;
                        }
                        mips_delayed_load(mipscpu, INS_RT(mipscpu->op), n_res);
                    }
            ...
            case OP_SB:
                if ((mipscpu->cp0r[CP0_SR] & SR_ISC) != 0) {
                    /* todo: */
                    logerror(mipscpu, "%08x: SB SR_ISC not supported\n", mipscpu->pc);
                    mips_stop(mipscpu);
                    mips_advance_pc(mipscpu);
                } else if ((mipscpu->cp0r[CP0_SR] & (SR_RE | SR_KUC)) == (SR_RE | SR_KUC)) {
                    UINT32 n_adr;
                    n_adr = mipscpu->r[INS_RS(mipscpu->op)] +
                            MIPS_WORD_EXTEND(INS_IMMEDIATE(mipscpu->op));
                    if ((n_adr & ((mipscpu->cp0r[CP0_SR] & SR_KUC) << 30)) != 0) {
                        mips_exception(mipscpu, EXC_ADES);
                        mips_set_cp0r(mipscpu, CP0_BADVADDR, n_adr);
                    } else {
                        program_write_byte_32le(mipscpu->psx, n_adr ^ 3,
                                                mipscpu->r[INS_RT(mipscpu->op)]);
                        mips_advance_pc(mipscpu);
                    }
                } else {
                    ...
                    if ((n_adr & ((mipscpu->cp0r[CP0_SR] & SR_KUC) << 30)) != 0) {
                        ...
                        program_write_byte_32le(mipscpu->psx, n_adr,
                                                mipscpu->r[INS_RT(mipscpu->op)]);
                        mips_advance_pc(mipscpu);
                    }
                }
            ...
            case OP_SH:
                if ((mipscpu->cp0r[CP0_SR] & SR_ISC) != 0) {
                    /* todo: */
                    logerror(mipscpu, "%08x: SH SR_ISC not supported\n", mipscpu->pc);
                    mips_stop(mipscpu);
                    mips_advance_pc(mipscpu);
                } else if ((mipscpu->cp0r[CP0_SR] & (SR_RE | SR_KUC)) == (SR_RE | SR_KUC)) {
                    UINT32 n_adr;
                    ...
                    if ((n_adr & (((mipscpu->cp0r[CP0_SR] & SR_KUC) << 30) | 1)) != 0) {
                        ...
                        program_write_word_32le(mipscpu->psx, n_adr ^ 2,
                                                mipscpu->r[INS_RT(mipscpu->op)]);
                        mips_advance_pc(mipscpu);
                    }
                } else {
                    ...
                    if ((n_adr & (((mipscpu->cp0r[CP0_SR] & SR_KUC) << 30) | 1)) != 0) {
                        ...
                        program_write_word_32le(mipscpu->psx, n_adr,
                                                mipscpu->r[INS_RT(mipscpu->op)]);
                        mips_advance_pc(mipscpu);
                    }
                }
            ...
            case OP_SWL:
                if ((mipscpu->cp0r[CP0_SR] & SR_ISC) != 0) {
                    /* todo: */
                    logerror(mipscpu, "%08x: SWL SR_ISC not supported\n", mipscpu->pc);
                    mips_stop(mipscpu);
                    mips_advance_pc(mipscpu);
                } else if ((mipscpu->cp0r[CP0_SR] & (SR_RE | SR_KUC)) == (SR_RE | SR_KUC)) {
                    UINT32 n_adr;
                    n_adr = mipscpu->r[INS_RS(mipscpu->op)] +
                            MIPS_WORD_EXTEND(INS_IMMEDIATE(mipscpu->op));
                    if ((n_adr & ((mipscpu->cp0r[CP0_SR] & SR_KUC) << 30)) != 0) {
                        logerror(mipscpu, "permission violation?\n");
                        mips_exception(mipscpu, EXC_ADES);
                        mips_set_cp0r(mipscpu, CP0_BADVADDR, n_adr);
                    } else {
                        switch (n_adr & 3) {
                            case 0:
                                program_write_byte_32le(mipscpu->psx, n_adr + 3,
                                                        mipscpu->r[INS_RT(mipscpu->op)] >> 24);
                                break;
                            case 1:
                                program_write_word_32le(mipscpu->psx, n_adr + 1,
                                                        mipscpu->r[INS_RT(mipscpu->op)] >> 16);
                                break;
                            case 2:
                                program_write_byte_32le(mipscpu->psx, n_adr - 1,
                                                        mipscpu->r[INS_RT(mipscpu->op)] >> 8);
                                program_write_word_32le(mipscpu->psx, n_adr,
                                                        mipscpu->r[INS_RT(mipscpu->op)] >> 16);
                                break;
                            case 3:
                                program_write_dword_32le(mipscpu->psx, n_adr - 3,
                                                         mipscpu->r[INS_RT(mipscpu->op)]);
                                break;
                        }
                        mips_advance_pc(mipscpu);
                    ...
                    n_adr = mipscpu->r[INS_RS(mipscpu->op)] +
                            MIPS_WORD_EXTEND(INS_IMMEDIATE(mipscpu->op));
                    if ((n_adr & ((mipscpu->cp0r[CP0_SR] & SR_KUC) << 30)) != 0) {
                        logerror(mipscpu, "permission violation 2\n");
                        mips_exception(mipscpu, EXC_ADES);
                        mips_set_cp0r(mipscpu, CP0_BADVADDR, n_adr);
                    } else {
                        switch (n_adr & 3) {
            ...
            case OP_SW:
                if ((mipscpu->cp0r[CP0_SR] & SR_ISC) != 0) {
                    /* todo: */
/* used by bootstrap
				logerror( mipscpu, "%08x: SW SR_ISC not supported\n", mipscpu->pc );
				mips_stop();
*/
                    mips_advance_pc(mipscpu);
                } else {
                    ...
                    if (0) // ( n_adr & ( ( ( mipscpu->cp0r[ CP0_SR ] & SR_KUC ) << 30 ) | 3 ) ) != 0 )
                    {
                        ...
                        program_write_dword_32le(mipscpu->psx, n_adr,
                                                 mipscpu->r[INS_RT(mipscpu->op)]);
                        mips_advance_pc(mipscpu);
                    }
                }
            ...
            case OP_SWR:
                if ((mipscpu->cp0r[CP0_SR] & SR_ISC) != 0) {
                    /* todo: */
                    logerror(mipscpu, "%08x: SWR SR_ISC not supported\n", mipscpu->pc);
                    mips_stop(mipscpu);
                    mips_advance_pc(mipscpu);
                } else if ((mipscpu->cp0r[CP0_SR] & (SR_RE | SR_KUC)) == (SR_RE | SR_KUC)) {
                    UINT32 n_adr;
            ...
            case OP_LWC1:
                /* todo: */
                logerror(mipscpu, "%08x: COP1 LWC not supported\n", mipscpu->pc);
                mips_stop(mipscpu);
                mips_advance_pc(mipscpu);
                break;
            case OP_LWC2:
                if ((mipscpu->cp0r[CP0_SR] & SR_CU2) == 0) {
                    ...
                    /* todo: */
                    logerror(mipscpu, "%08x: LWC2 SR_ISC not supported\n", mipscpu->pc);
                    mips_stop(mipscpu);
                    mips_advance_pc(mipscpu);
                } else {
                    UINT32 n_adr;
                    ...
                    if ((n_adr & (((mipscpu->cp0r[CP0_SR] & SR_KUC) << 30) | 3)) != 0) {
                        ...
                        /* todo: delay? */
                        setcp2dr(mipscpu, INS_RT(mipscpu->op),
                                 program_read_dword_32le(mipscpu->psx, n_adr));
                        mips_advance_pc(mipscpu);
                    }
                }
            ...
            case OP_SWC1:
                /* todo: */
                logerror(mipscpu, "%08x: COP1 SWC not supported\n", mipscpu->pc);
                mips_stop(mipscpu);
                mips_advance_pc(mipscpu);
                break;
            case OP_SWC2:
                if ((mipscpu->cp0r[CP0_SR] & SR_CU2) == 0) {
                    ...
                    /* todo: */
                    logerror(mipscpu, "%08x: SWC2 SR_ISC not supported\n", mipscpu->pc);
                    mips_stop(mipscpu);
                    mips_advance_pc(mipscpu);
                } else {
                    UINT32 n_adr;
                    ...
                    if ((n_adr & (((mipscpu->cp0r[CP0_SR] & SR_KUC) << 30) | 3)) != 0) {
                        ...
                        program_write_dword_32le(mipscpu->psx, n_adr,
                                                 getcp2dr(mipscpu, INS_RT(mipscpu->op)));
                        mips_advance_pc(mipscpu);
                    }
                }
                break;
...
//			mips_exception( EXC_RI );
        ...
        skipinterp:
        mipscpu->mips_ICount--;
    } while (mipscpu->mips_ICount > 0);

    return cycles - mipscpu->mips_ICount;
}

static void mips_get_context(MIPS_CPU_CONTEXT *mipscpu, void *dst) {
    if (dst) {
        *(struct mips_cpu_context *) dst = *mipscpu;
    }
}

static void mips_set_context(MIPS_CPU_CONTEXT *mipscpu, void *src) {
    if (src) {
        *mipscpu = *(struct mips_cpu_context *) src;
        change_pc(mipscpu->pc);
    }
}

static void set_irq_line(MIPS_CPU_CONTEXT *mipscpu, int irqline, int state) {
    UINT32 ip;
    ...
    switch (state) {
        case CLEAR_LINE:
            mips_set_cp0r(mipscpu, CP0_CAUSE, mipscpu->cp0r[CP0_CAUSE] & ~ip);
            break;
        case ASSERT_LINE:
            mips_set_cp0r(mipscpu, CP0_CAUSE, mipscpu->cp0r[CP0_CAUSE] |= ip);
			...
			the interrupt. */
                (*mipscpu->irq_callback)(mipscpu->irq_callback_param, irqline);
            }
            break;
    }
}
...
offs_t mips_dasm(MIPS_CPU_CONTEXT *mipscpu, char *buffer, offs_t pc) {
    offs_t ret;
    change_pc(pc);
#ifdef MAME_DEBUG
...
static UINT32 getcp2dr(MIPS_CPU_CONTEXT *mipscpu, int n_reg) {
    if (n_reg == 1 || n_reg == 3 || n_reg == 5 || n_reg == 8 || n_reg == 9 || n_reg == 10 ||
        n_reg == 11) {
...
INLINE INT32
LIM(MIPS_CPU_CONTEXT *mipscpu, INT32 n_value, INT32 n_max, INT32 n_min, UINT32 n_flag) {
    if (n_value > n_max) {
        FLAG |= n_flag;
        return n_max;
...
INLINE INT64
BOUNDS(MIPS_CPU_CONTEXT *mipscpu, INT64 n_value, INT64 n_max, int n_maxflag, INT64 n_min,
       int n_minflag) {
    if (n_value > n_max) {
        FLAG |= n_maxflag;
    } else if (n_value < n_min) {
...
#define A1(a) BOUNDS( mipscpu, ( a ), 0x7fffffff, 30, -(INT64)0x80000000, ( 1 << 27 ) )
#define A2(a) BOUNDS( mipscpu, ( a ), 0x7fffffff, 29, -(INT64)0x80000000, ( 1 << 26 ) )
#define A3(a) BOUNDS( mipscpu, ( a ), 0x7fffffff, 28, -(INT64)0x80000000, ( 1 << 25 ) )
#define Lm_B1(a, l) LIM( mipscpu, ( a ), 0x7fff, -0x8000 * !l, ( 1 << 31 ) | ( 1 << 24 ) )
#define Lm_B2(a, l) LIM( mipscpu, ( a ), 0x7fff, -0x8000 * !l, ( 1 << 31 ) | ( 1 << 23 ) )
#define Lm_B3(a, l) LIM( mipscpu, ( a ), 0x7fff, -0x8000 * !l, ( 1 << 22 ) )
#define Lm_C1(a) LIM( mipscpu, ( a ), 0x00ff, 0x0000, ( 1 << 21 ) )
#define Lm_C2(a) LIM( mipscpu, ( a ), 0x00ff, 0x0000, ( 1 << 20 ) )
#define Lm_C3(a) LIM( mipscpu, ( a ), 0x00ff, 0x0000, ( 1 << 19 ) )
#define Lm_D(a) LIM( mipscpu, ( a ), 0xffff, 0x0000, ( 1 << 31 ) | ( 1 << 18 ) )

INLINE UINT32 Lm_E(MIPS_CPU_CONTEXT *mipscpu, UINT32 n_z) {
    if (n_z <= H / 2) {
        n_z = H / 2;
        FLAG |= (1 << 31) | (1 << 17);
    }
...
#define F(a) BOUNDS( mipscpu, ( a ), 0x7fffffff, ( 1 << 31 ) | ( 1 << 16 ), -(INT64)0x80000000, ( 1 << 31 ) | ( 1 << 15 ) )
#define Lm_G1(a) LIM( mipscpu, ( a ), 0x3ff, -0x400, ( 1 << 31 ) | ( 1 << 14 ) )
#define Lm_G2(a) LIM( mipscpu, ( a ), 0x3ff, -0x400, ( 1 << 31 ) | ( 1 << 13 ) )
#define Lm_H(a) LIM( mipscpu, ( a ), 0xfff, 0x000, ( 1 << 12 ) )
...
static void docop2(MIPS_CPU_CONTEXT *mipscpu, int gteop) {
    ...
    const UINT16 n_zm = 0;
    const UINT32 n_zc = 0;
    const UINT16 *p_n_vx[] = {&VX0, &VX1, &VX2};
    const UINT16 *p_n_vy[] = {&VY0, &VY1, &VY2};
    const UINT16 *p_n_vz[] = {&VZ0, &VZ1, &VZ2};
    const UINT16 *p_n_rm[] = {&R11, &R12, &R13, &R21, &R22, &R23, &R31, &R32, &R33};
    const UINT16 *p_n_lm[] = {&L11, &L12, &L13, &L21, &L22, &L23, &L31, &L32, &L33};
    const UINT16 *p_n_cm[] = {&LR1, &LR2, &LR3, &LG1, &LG2, &LG3, &LB1, &LB2, &LB3};
    const UINT16 *p_n_zm[] = {&n_zm, &n_zm, &n_zm, &n_zm, &n_zm, &n_zm, &n_zm, &n_zm, &n_zm};
    const UINT16 **p_p_n_mx[] = {p_n_rm, p_n_lm, p_n_cm, p_n_zm};
    const UINT32 *p_n_tr[] = {&TRX, &TRY, &TRZ};
    const UINT32 *p_n_bk[] = {&RBK, &GBK, &BBK};
    const UINT32 *p_n_fc[] = {&RFC, &GFC, &BFC};
    const UINT32 *p_n_zc[] = {&n_zc, &n_zc, &n_zc};
    const UINT32 **p_p_n_cv[] = {p_n_tr, p_n_bk, p_n_fc, p_n_zc};
    ...
    switch (GTE_FUNCT(gteop)) {
        case 0x01:
            if (gteop == 0x0180001) {
                ...
                MAC1 = A1((((INT64) (INT32) TRX << 12) + ((INT16) R11 * (INT16) VX0) +
                           ((INT16) R12 * (INT16) VY0) + ((INT16) R13 * (INT16) VZ0)) >> 12);
                MAC2 = A2((((INT64) (INT32) TRY << 12) + ((INT16) R21 * (INT16) VX0) +
                           ((INT16) R22 * (INT16) VY0) + ((INT16) R23 * (INT16) VZ0)) >> 12);
                MAC3 = A3((((INT64) (INT32) TRZ << 12) + ((INT16) R31 * (INT16) VX0) +
                           ((INT16) R32 * (INT16) VY0) + ((INT16) R33 * (INT16) VZ0)) >> 12);
                ...
                SXY0 = SXY1;
                SXY1 = SXY2;
                SX2 = Lm_G1(F((INT64) (INT32) OFX +
                              ((INT64) (INT16) IR1 * (((UINT32) H << 16) / Lm_E(mipscpu, SZ3))))
                                    >> 16);
                SY2 = Lm_G2(F((INT64) (INT32) OFY +
                              ((INT64) (INT16) IR2 * (((UINT32) H << 16) / Lm_E(mipscpu, SZ3))))
                                    >> 16);
                MAC0 = F((INT64) (INT32) DQB +
                         ((INT64) (INT16) DQA * (((UINT32) H << 16) / Lm_E(mipscpu, SZ3))));
        ...
        case 0x06:
            if (gteop == 0x0400006 ||
                ...
                MAC0 = F(((INT64) (INT16) SX0 * (INT16) SY1) + ((INT16) SX1 * (INT16) SY2) +
                         ((INT16) SX2 * (INT16) SY0) - ((INT16) SX0 * (INT16) SY2) -
                         ((INT16) SX1 * (INT16) SY0) - ((INT16) SX2 * (INT16) SY1));
                return;
            }
            break;
        case 0x0c:
            if (GTE_OP(gteop) == 0x17) {
                ...
                MAC1 = A1((((INT64) (INT32) D2 * (INT16) IR3) - ((INT64) (INT32) D3 * (INT16) IR2))
                                  >> n_sf);
                MAC2 = A2((((INT64) (INT32) D3 * (INT16) IR1) - ((INT64) (INT32) D1 * (INT16) IR3))
                                  >> n_sf);
                MAC3 = A3((((INT64) (INT32) D1 * (INT16) IR2) - ((INT64) (INT32) D2 * (INT16) IR1))
                                  >> n_sf);
                IR1 = Lm_B1((INT32) MAC1, 0);
                IR2 = Lm_B2((INT32) MAC2, 0);
                IR3 = Lm_B3((INT32) MAC3, 0);
        ...
        case 0x10:
            if (gteop == 0x0780010) {
                ...
                MAC1 = A1((((INT64) R << 16) +
                           ((INT64) (INT16) IR0 * (Lm_B1((INT32) RFC - (R << 4), 0)))) >> 12);
                MAC2 = A2((((INT64) G << 16) +
                           ((INT64) (INT16) IR0 * (Lm_B1((INT32) GFC - (G << 4), 0)))) >> 12);
                MAC3 = A3((((INT64) B << 16) +
                           ((INT64) (INT16) IR0 * (Lm_B1((INT32) BFC - (B << 4), 0)))) >> 12);
                IR1 = Lm_B1((INT32) MAC1, 0);
                IR2 = Lm_B2((INT32) MAC2, 0);
                IR3 = Lm_B3((INT32) MAC3, 0);
                CD0 = CD1;
        ...
        case 0x11:
            if (gteop == 0x0980011) {
                ...
                MAC1 = A1((((INT64) (INT16) IR1 << 12) +
                           ((INT64) (INT16) IR0 * (Lm_B1((INT32) RFC - (INT16) IR1, 0)))) >> 12);
                MAC2 = A2((((INT64) (INT16) IR2 << 12) +
                           ((INT64) (INT16) IR0 * (Lm_B1((INT32) GFC - (INT16) IR2, 0)))) >> 12);
                MAC3 = A3((((INT64) (INT16) IR3 << 12) +
                           ((INT64) (INT16) IR0 * (Lm_B1((INT32) BFC - (INT16) IR3, 0)))) >> 12);
                IR1 = Lm_B1((INT32) MAC1, 0);
                IR2 = Lm_B2((INT32) MAC2, 0);
                IR3 = Lm_B3((INT32) MAC3, 0);
                CD0 = CD1;
        ...
        case 0x12:
            if (GTE_OP(gteop) == 0x04) {
                ...
                MAC1 = A1(
                        (((INT64) (INT32) *p_n_cv[0] << 12) + ((INT16) *p_n_mx[0] * (INT16) n_v1) +
                         ((INT16) *p_n_mx[1] * (INT16) n_v2) + ((INT16) *p_n_mx[2] * (INT16) n_v3))
                                >> n_sf);
                MAC2 = A2(
                        (((INT64) (INT32) *p_n_cv[1] << 12) + ((INT16) *p_n_mx[3] * (INT16) n_v1) +
                         ((INT16) *p_n_mx[4] * (INT16) n_v2) + ((INT16) *p_n_mx[5] * (INT16) n_v3))
                                >> n_sf);
                MAC3 = A3(
                        (((INT64) (INT32) *p_n_cv[2] << 12) + ((INT16) *p_n_mx[6] * (INT16) n_v1) +
                         ((INT16) *p_n_mx[7] * (INT16) n_v2) + ((INT16) *p_n_mx[8] * (INT16) n_v3))
                                >> n_sf);
        ...
        case 0x13:
            if (gteop == 0x0e80413) {
                ...
                MAC1 = A1((((INT64) (INT16) L11 * (INT16) VX0) + ((INT16) L12 * (INT16) VY0) +
                           ((INT16) L13 * (INT16) VZ0)) >> 12);
                MAC2 = A2((((INT64) (INT16) L21 * (INT16) VX0) + ((INT16) L22 * (INT16) VY0) +
                           ((INT16) L23 * (INT16) VZ0)) >> 12);
                MAC3 = A3((((INT64) (INT16) L31 * (INT16) VX0) + ((INT16) L32 * (INT16) VY0) +
                           ((INT16) L33 * (INT16) VZ0)) >> 12);
                IR1 = Lm_B1((INT32) MAC1, 1);
                IR2 = Lm_B2((INT32) MAC2, 1);
                IR3 = Lm_B3((INT32) MAC3, 1);
                MAC1 = A1((((INT64) RBK << 12) + ((INT16) LR1 * (INT16) IR1) +
                           ((INT16) LR2 * (INT16) IR2) + ((INT16) LR3 * (INT16) IR3)) >> 12);
                MAC2 = A2((((INT64) GBK << 12) + ((INT16) LG1 * (INT16) IR1) +
                           ((INT16) LG2 * (INT16) IR2) + ((INT16) LG3 * (INT16) IR3)) >> 12);
                MAC3 = A3((((INT64) BBK << 12) + ((INT16) LB1 * (INT16) IR1) +
                           ((INT16) LB2 * (INT16) IR2) + ((INT16) LB3 * (INT16) IR3)) >> 12);
                IR1 = Lm_B1((INT32) MAC1, 1);
                IR2 = Lm_B2((INT32) MAC2, 1);
                IR3 = Lm_B3((INT32) MAC3, 1);
                MAC1 = A1(((((INT64) R << 4) * (INT16) IR1) +
                           ((INT16) IR0 * Lm_B1((INT32) RFC - ((R * (INT16) IR1) >> 8), 0))) >> 12);
                MAC2 = A2(((((INT64) G << 4) * (INT16) IR2) +
                           ((INT16) IR0 * Lm_B2((INT32) GFC - ((G * (INT16) IR2) >> 8), 0))) >> 12);
                MAC3 = A3(((((INT64) B << 4) * (INT16) IR3) +
                           ((INT16) IR0 * Lm_B3((INT32) BFC - ((B * (INT16) IR3) >> 8), 0))) >> 12);
                IR1 = Lm_B1((INT32) MAC1, 1);
                IR2 = Lm_B2((INT32) MAC2, 1);
                IR3 = Lm_B3((INT32) MAC3, 1);
                CD0 = CD1;
        ...
        case 0x16:
            if (gteop == 0x0f80416) {
                ...
                for (n_v = 0; n_v < 3; n_v++) {
                    MAC1 = A1((((INT64) (INT16) L11 * (INT16) *p_n_vx[n_v]) +
                               ((INT16) L12 * (INT16) *p_n_vy[n_v]) +
                               ((INT16) L13 * (INT16) *p_n_vz[n_v])) >> 12);
                    MAC2 = A2((((INT64) (INT16) L21 * (INT16) *p_n_vx[n_v]) +
                               ((INT16) L22 * (INT16) *p_n_vy[n_v]) +
                               ((INT16) L23 * (INT16) *p_n_vz[n_v])) >> 12);
                    MAC3 = A3((((INT64) (INT16) L31 * (INT16) *p_n_vx[n_v]) +
                               ((INT16) L32 * (INT16) *p_n_vy[n_v]) +
                               ((INT16) L33 * (INT16) *p_n_vz[n_v])) >> 12);
                    IR1 = Lm_B1((INT32) MAC1, 1);
                    IR2 = Lm_B2((INT32) MAC2, 1);
                    IR3 = Lm_B3((INT32) MAC3, 1);
                    MAC1 = A1((((INT64) RBK << 12) + ((INT16) LR1 * (INT16) IR1) +
                    ...
                    IR3 = Lm_B3((INT32) MAC3, 1);
                    MAC1 = A1(((((INT64) R << 4) * (INT16) IR1) +
                               ((INT16) IR0 * Lm_B1((INT32) RFC - ((R * (INT16) IR1) >> 8), 0)))
                                      >> 12);
                    MAC2 = A2(((((INT64) G << 4) * (INT16) IR2) +
                               ((INT16) IR0 * Lm_B2((INT32) GFC - ((G * (INT16) IR2) >> 8), 0)))
                                      >> 12);
                    MAC3 = A3(((((INT64) B << 4) * (INT16) IR3) +
                               ((INT16) IR0 * Lm_B3((INT32) BFC - ((B * (INT16) IR3) >> 8), 0)))
                                      >> 12);
                    IR1 = Lm_B1((INT32) MAC1, 1);
                    IR2 = Lm_B2((INT32) MAC2, 1);
                    IR3 = Lm_B3((INT32) MAC3, 1);
        ...
        case 0x2a:
            if (gteop == 0x0f8002a) {
                ...
                for (n_pass = 0; n_pass < 3; n_pass++) {
                    MAC1 = A1((((INT64) R0 << 16) +
                               ((INT64) (INT16) IR0 * (Lm_B1((INT32) RFC - (R0 << 4), 0)))) >> 12);
                    MAC2 = A2((((INT64) G0 << 16) +
                               ((INT64) (INT16) IR0 * (Lm_B1((INT32) GFC - (G0 << 4), 0)))) >> 12);
                    MAC3 = A3((((INT64) B0 << 16) +
                               ((INT64) (INT16) IR0 * (Lm_B1((INT32) BFC - (B0 << 4), 0)))) >> 12);
                    IR1 = Lm_B1((INT32) MAC1, 0);
                    IR2 = Lm_B2((INT32) MAC2, 0);
                    IR3 = Lm_B3((INT32) MAC3, 0);
                    CD0 = CD1;
        ...
        case 0x2d:
            if (gteop == 0x158002d) {
                ...
                MAC0 = F(
                        ((INT64) (INT16) ZSF3 * SZ1) + ((INT16) ZSF3 * SZ2) + ((INT16) ZSF3 * SZ3));
        ...
        case 0x2e:
            if (gteop == 0x168002e) {
                ...
                MAC0 = F(
                        ((INT64) (INT16) ZSF4 * SZ0) + ((INT16) ZSF4 * SZ1) + ((INT16) ZSF4 * SZ2) +
                        ((INT16) ZSF4 * SZ3));
                OTZ = Lm_D((INT32) MAC0 >> 12);
                return;
            }
        ...
        case 0x30:
            if (gteop == 0x0280030) {
                ...
                for (n_v = 0; n_v < 3; n_v++) {
                    MAC1 = A1((((INT64) (INT32) TRX << 12) + ((INT16) R11 * (INT16) *p_n_vx[n_v]) +
                               ((INT16) R12 * (INT16) *p_n_vy[n_v]) +
                               ((INT16) R13 * (INT16) *p_n_vz[n_v])) >> 12);
                    MAC2 = A2((((INT64) (INT32) TRY << 12) + ((INT16) R21 * (INT16) *p_n_vx[n_v]) +
                               ((INT16) R22 * (INT16) *p_n_vy[n_v]) +
                               ((INT16) R23 * (INT16) *p_n_vz[n_v])) >> 12);
                    MAC3 = A3((((INT64) (INT32) TRZ << 12) + ((INT16) R31 * (INT16) *p_n_vx[n_v]) +
                               ((INT16) R32 * (INT16) *p_n_vy[n_v]) +
                               ((INT16) R33 * (INT16) *p_n_vz[n_v])) >> 12);
                    IR1 = Lm_B1((INT32) MAC1, 0);
                    IR2 = Lm_B2((INT32) MAC2, 0);
                    IR3 = Lm_B3((INT32) MAC3, 0);
                    SZ0 = SZ1;
                    ...
                    SXY0 = SXY1;
                    SXY1 = SXY2;
                    SX2 = Lm_G1(F(((INT64) (INT32) OFX + ((INT64) (INT16) IR1 *
                                                          (((UINT32) H << 16) /
                                                           Lm_E(mipscpu, SZ3)))) >> 16));
                    SY2 = Lm_G2(F(((INT64) (INT32) OFY + ((INT64) (INT16) IR2 *
                                                          (((UINT32) H << 16) /
                                                           Lm_E(mipscpu, SZ3)))) >> 16));
                    MAC0 = F((INT64) (INT32) DQB +
                             ((INT64) (INT16) DQA * (((UINT32) H << 16) / Lm_E(mipscpu, SZ3))));
...
//	usrintf_showmessage_secs( 1, "unknown GTE op %08x", gteop );
    logerror(mipscpu, "%08x: unknown GTE op %08x\n", mipscpu->pc, gteop);
    mips_stop(mipscpu);
}
...
void mips_set_info(MIPS_CPU_CONTEXT *mipscpu, UINT32 state, union cpuinfo *info) {
    switch (state) {
        /* --- the following bits of info are set as 64-bit signed integers --- */
        case CPUINFO_INT_INPUT_STATE + MIPS_IRQ0:
            set_irq_line(mipscpu, MIPS_IRQ0, info->i);
            break;
        case CPUINFO_INT_INPUT_STATE + MIPS_IRQ1:
            set_irq_line(mipscpu, MIPS_IRQ1, info->i);
            break;
        case CPUINFO_INT_INPUT_STATE + MIPS_IRQ2:
            set_irq_line(mipscpu, MIPS_IRQ2, info->i);
            break;
        case CPUINFO_INT_INPUT_STATE + MIPS_IRQ3:
            set_irq_line(mipscpu, MIPS_IRQ3, info->i);
            break;
        case CPUINFO_INT_INPUT_STATE + MIPS_IRQ4:
            set_irq_line(mipscpu, MIPS_IRQ4, info->i);
            break;
        case CPUINFO_INT_INPUT_STATE + MIPS_IRQ5:
            set_irq_line(mipscpu, MIPS_IRQ5, info->i);
            break;

        case CPUINFO_INT_PC:
            mips_set_pc(mipscpu, info->i);
            break;
        case CPUINFO_INT_REGISTER + MIPS_PC:
            mips_set_pc(mipscpu, info->i);
            break;
        case CPUINFO_INT_SP:                            /* no stack */                            break;
        case CPUINFO_INT_REGISTER + MIPS_DELAYV:
            mipscpu->delayv = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_DELAYR:
            if (info->i <= REGPC) mipscpu->delayr = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_HI:
            mipscpu->hi = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_LO:
            mipscpu->lo = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_R0:
            mipscpu->r[0] = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_R1:
            mipscpu->r[1] = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_R2:
            mipscpu->r[2] = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_R3:
            mipscpu->r[3] = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_R4:
            mipscpu->r[4] = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_R5:
            mipscpu->r[5] = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_R6:
            mipscpu->r[6] = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_R7:
            mipscpu->r[7] = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_R8:
            mipscpu->r[8] = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_R9:
            mipscpu->r[9] = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_R10:
            mipscpu->r[10] = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_R11:
            mipscpu->r[11] = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_R12:
            mipscpu->r[12] = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_R13:
            mipscpu->r[13] = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_R14:
            mipscpu->r[14] = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_R15:
            mipscpu->r[15] = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_R16:
            mipscpu->r[16] = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_R17:
            mipscpu->r[17] = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_R18:
            mipscpu->r[18] = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_R19:
            mipscpu->r[19] = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_R20:
            mipscpu->r[20] = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_R21:
            mipscpu->r[21] = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_R22:
            mipscpu->r[22] = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_R23:
            mipscpu->r[23] = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_R24:
            mipscpu->r[24] = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_R25:
            mipscpu->r[25] = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_R26:
            mipscpu->r[26] = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_R27:
            mipscpu->r[27] = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_R28:
            mipscpu->r[28] = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_R29:
            mipscpu->r[29] = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_R30:
            mipscpu->r[30] = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_R31:
            mipscpu->r[31] = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP0R0:
            mips_set_cp0r(mipscpu, 0, info->i);
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP0R1:
            mips_set_cp0r(mipscpu, 1, info->i);
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP0R2:
            mips_set_cp0r(mipscpu, 2, info->i);
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP0R3:
            mips_set_cp0r(mipscpu, 3, info->i);
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP0R4:
            mips_set_cp0r(mipscpu, 4, info->i);
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP0R5:
            mips_set_cp0r(mipscpu, 5, info->i);
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP0R6:
            mips_set_cp0r(mipscpu, 6, info->i);
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP0R7:
            mips_set_cp0r(mipscpu, 7, info->i);
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP0R8:
            mips_set_cp0r(mipscpu, 8, info->i);
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP0R9:
            mips_set_cp0r(mipscpu, 9, info->i);
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP0R10:
            mips_set_cp0r(mipscpu, 10, info->i);
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP0R11:
            mips_set_cp0r(mipscpu, 11, info->i);
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP0R12:
            mips_set_cp0r(mipscpu, 12, info->i);
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP0R13:
            mips_set_cp0r(mipscpu, 13, info->i);
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP0R14:
            mips_set_cp0r(mipscpu, 14, info->i);
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP0R15:
            mips_set_cp0r(mipscpu, 15, info->i);
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP0R16:
            mips_set_cp0r(mipscpu, 16, info->i);
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP0R17:
            mips_set_cp0r(mipscpu, 17, info->i);
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP0R18:
            mips_set_cp0r(mipscpu, 18, info->i);
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP0R19:
            mips_set_cp0r(mipscpu, 19, info->i);
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP0R20:
            mips_set_cp0r(mipscpu, 20, info->i);
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP0R21:
            mips_set_cp0r(mipscpu, 21, info->i);
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP0R22:
            mips_set_cp0r(mipscpu, 22, info->i);
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP0R23:
            mips_set_cp0r(mipscpu, 23, info->i);
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP0R24:
            mips_set_cp0r(mipscpu, 24, info->i);
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP0R25:
            mips_set_cp0r(mipscpu, 25, info->i);
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP0R26:
            mips_set_cp0r(mipscpu, 26, info->i);
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP0R27:
            mips_set_cp0r(mipscpu, 27, info->i);
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP0R28:
            mips_set_cp0r(mipscpu, 28, info->i);
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP0R29:
            mips_set_cp0r(mipscpu, 29, info->i);
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP0R30:
            mips_set_cp0r(mipscpu, 30, info->i);
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP0R31:
            mips_set_cp0r(mipscpu, 31, info->i);
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2DR0:
            mipscpu->cp2dr[0].d = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2DR1:
            mipscpu->cp2dr[1].d = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2DR2:
            mipscpu->cp2dr[2].d = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2DR3:
            mipscpu->cp2dr[3].d = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2DR4:
            mipscpu->cp2dr[4].d = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2DR5:
            mipscpu->cp2dr[5].d = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2DR6:
            mipscpu->cp2dr[6].d = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2DR7:
            mipscpu->cp2dr[7].d = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2DR8:
            mipscpu->cp2dr[8].d = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2DR9:
            mipscpu->cp2dr[9].d = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2DR10:
            mipscpu->cp2dr[10].d = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2DR11:
            mipscpu->cp2dr[11].d = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2DR12:
            mipscpu->cp2dr[12].d = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2DR13:
            mipscpu->cp2dr[13].d = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2DR14:
            mipscpu->cp2dr[14].d = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2DR15:
            mipscpu->cp2dr[15].d = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2DR16:
            mipscpu->cp2dr[16].d = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2DR17:
            mipscpu->cp2dr[17].d = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2DR18:
            mipscpu->cp2dr[18].d = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2DR19:
            mipscpu->cp2dr[19].d = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2DR20:
            mipscpu->cp2dr[20].d = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2DR21:
            mipscpu->cp2dr[21].d = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2DR22:
            mipscpu->cp2dr[22].d = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2DR23:
            mipscpu->cp2dr[23].d = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2DR24:
            mipscpu->cp2dr[24].d = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2DR25:
            mipscpu->cp2dr[25].d = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2DR26:
            mipscpu->cp2dr[26].d = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2DR27:
            mipscpu->cp2dr[27].d = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2DR28:
            mipscpu->cp2dr[28].d = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2DR29:
            mipscpu->cp2dr[29].d = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2DR30:
            mipscpu->cp2dr[30].d = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2DR31:
            mipscpu->cp2dr[31].d = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2CR0:
            mipscpu->cp2cr[0].d = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2CR1:
            mipscpu->cp2cr[1].d = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2CR2:
            mipscpu->cp2cr[2].d = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2CR3:
            mipscpu->cp2cr[3].d = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2CR4:
            mipscpu->cp2cr[4].d = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2CR5:
            mipscpu->cp2cr[5].d = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2CR6:
            mipscpu->cp2cr[6].d = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2CR7:
            mipscpu->cp2cr[7].d = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2CR8:
            mipscpu->cp2cr[8].d = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2CR9:
            mipscpu->cp2cr[9].d = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2CR10:
            mipscpu->cp2cr[10].d = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2CR11:
            mipscpu->cp2cr[11].d = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2CR12:
            mipscpu->cp2cr[12].d = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2CR13:
            mipscpu->cp2cr[13].d = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2CR14:
            mipscpu->cp2cr[14].d = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2CR15:
            mipscpu->cp2cr[15].d = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2CR16:
            mipscpu->cp2cr[16].d = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2CR17:
            mipscpu->cp2cr[17].d = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2CR18:
            mipscpu->cp2cr[18].d = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2CR19:
            mipscpu->cp2cr[19].d = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2CR20:
            mipscpu->cp2cr[20].d = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2CR21:
            mipscpu->cp2cr[21].d = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2CR22:
            mipscpu->cp2cr[22].d = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2CR23:
            mipscpu->cp2cr[23].d = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2CR24:
            mipscpu->cp2cr[24].d = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2CR25:
            mipscpu->cp2cr[25].d = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2CR26:
            mipscpu->cp2cr[26].d = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2CR27:
            mipscpu->cp2cr[27].d = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2CR28:
            mipscpu->cp2cr[28].d = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2CR29:
            mipscpu->cp2cr[29].d = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2CR30:
            mipscpu->cp2cr[30].d = info->i;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2CR31:
            mipscpu->cp2cr[31].d = info->i;
            break;

            /* --- the following bits of info are set as pointers to data or functions --- */
        case CPUINFO_PTR_IRQ_CALLBACK:
            mipscpu->irq_callback = info->irqcallback;
            break;
    }
}
...
void mips_get_info(MIPS_CPU_CONTEXT *mipscpu, UINT32 state, union cpuinfo *info) {
    switch (state) {
        /* --- the following bits of info are returned as 64-bit signed integers --- */
        case CPUINFO_INT_CONTEXT_SIZE:
            info->i = sizeof(*mipscpu);
            break;
        ...
        case CPUINFO_INT_INPUT_STATE + MIPS_IRQ0:
            info->i = (mipscpu->cp0r[CP0_CAUSE] & 0x400) ? ASSERT_LINE : CLEAR_LINE;
            break;
        case CPUINFO_INT_INPUT_STATE + MIPS_IRQ1:
            info->i = (mipscpu->cp0r[CP0_CAUSE] & 0x800) ? ASSERT_LINE : CLEAR_LINE;
            break;
        case CPUINFO_INT_INPUT_STATE + MIPS_IRQ2:
            info->i = (mipscpu->cp0r[CP0_CAUSE] & 0x1000) ? ASSERT_LINE : CLEAR_LINE;
            break;
        case CPUINFO_INT_INPUT_STATE + MIPS_IRQ3:
            info->i = (mipscpu->cp0r[CP0_CAUSE] & 0x2000) ? ASSERT_LINE : CLEAR_LINE;
            break;
        case CPUINFO_INT_INPUT_STATE + MIPS_IRQ4:
            info->i = (mipscpu->cp0r[CP0_CAUSE] & 0x4000) ? ASSERT_LINE : CLEAR_LINE;
            break;
        case CPUINFO_INT_INPUT_STATE + MIPS_IRQ5:
            info->i = (mipscpu->cp0r[CP0_CAUSE] & 0x8000) ? ASSERT_LINE : CLEAR_LINE;
            break;
        ...
        case CPUINFO_INT_SP:
            ...
            break;
        case CPUINFO_INT_REGISTER + MIPS_DELAYV:
            info->i = mipscpu->delayv;
            break;
        case CPUINFO_INT_REGISTER + MIPS_DELAYR:
            info->i = mipscpu->delayr;
            break;
        case CPUINFO_INT_REGISTER + MIPS_HI:
            info->i = mipscpu->hi;
            break;
        ...
        case CPUINFO_INT_REGISTER + MIPS_R31:
            ...
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP0R0:
            info->i = mipscpu->cp0r[0];
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP0R1:
            info->i = mipscpu->cp0r[1];
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP0R2:
            info->i = mipscpu->cp0r[2];
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP0R3:
            info->i = mipscpu->cp0r[3];
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP0R4:
            info->i = mipscpu->cp0r[4];
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP0R5:
            info->i = mipscpu->cp0r[5];
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP0R6:
            info->i = mipscpu->cp0r[6];
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP0R7:
            info->i = mipscpu->cp0r[7];
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP0R8:
            info->i = mipscpu->cp0r[8];
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP0R9:
            info->i = mipscpu->cp0r[9];
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP0R10:
            info->i = mipscpu->cp0r[10];
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP0R11:
            info->i = mipscpu->cp0r[11];
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP0R12:
            info->i = mipscpu->cp0r[12];
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP0R13:
            info->i = mipscpu->cp0r[13];
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP0R14:
            info->i = mipscpu->cp0r[14];
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP0R15:
            info->i = mipscpu->cp0r[15];
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP0R16:
            info->i = mipscpu->cp0r[16];
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP0R17:
            info->i = mipscpu->cp0r[17];
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP0R18:
            info->i = mipscpu->cp0r[18];
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP0R19:
            info->i = mipscpu->cp0r[19];
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP0R20:
            info->i = mipscpu->cp0r[20];
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP0R21:
            info->i = mipscpu->cp0r[21];
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP0R22:
            info->i = mipscpu->cp0r[22];
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP0R23:
            info->i = mipscpu->cp0r[23];
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP0R24:
            info->i = mipscpu->cp0r[24];
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP0R25:
            info->i = mipscpu->cp0r[25];
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP0R26:
            info->i = mipscpu->cp0r[26];
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP0R27:
            info->i = mipscpu->cp0r[27];
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP0R28:
            info->i = mipscpu->cp0r[28];
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP0R29:
            info->i = mipscpu->cp0r[29];
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP0R30:
            info->i = mipscpu->cp0r[30];
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP0R31:
            info->i = mipscpu->cp0r[31];
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2DR0:
            info->i = mipscpu->cp2dr[0].d;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2DR1:
            info->i = mipscpu->cp2dr[1].d;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2DR2:
            info->i = mipscpu->cp2dr[2].d;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2DR3:
            info->i = mipscpu->cp2dr[3].d;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2DR4:
            info->i = mipscpu->cp2dr[4].d;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2DR5:
            info->i = mipscpu->cp2dr[5].d;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2DR6:
            info->i = mipscpu->cp2dr[6].d;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2DR7:
            info->i = mipscpu->cp2dr[7].d;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2DR8:
            info->i = mipscpu->cp2dr[8].d;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2DR9:
            info->i = mipscpu->cp2dr[9].d;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2DR10:
            info->i = mipscpu->cp2dr[10].d;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2DR11:
            info->i = mipscpu->cp2dr[11].d;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2DR12:
            info->i = mipscpu->cp2dr[12].d;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2DR13:
            info->i = mipscpu->cp2dr[13].d;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2DR14:
            info->i = mipscpu->cp2dr[14].d;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2DR15:
            info->i = mipscpu->cp2dr[15].d;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2DR16:
            info->i = mipscpu->cp2dr[16].d;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2DR17:
            info->i = mipscpu->cp2dr[17].d;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2DR18:
            info->i = mipscpu->cp2dr[18].d;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2DR19:
            info->i = mipscpu->cp2dr[19].d;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2DR20:
            info->i = mipscpu->cp2dr[20].d;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2DR21:
            info->i = mipscpu->cp2dr[21].d;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2DR22:
            info->i = mipscpu->cp2dr[22].d;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2DR23:
            info->i = mipscpu->cp2dr[23].d;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2DR24:
            info->i = mipscpu->cp2dr[24].d;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2DR25:
            info->i = mipscpu->cp2dr[25].d;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2DR26:
            info->i = mipscpu->cp2dr[26].d;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2DR27:
            info->i = mipscpu->cp2dr[27].d;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2DR28:
            info->i = mipscpu->cp2dr[28].d;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2DR29:
            info->i = mipscpu->cp2dr[29].d;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2DR30:
            info->i = mipscpu->cp2dr[30].d;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2DR31:
            info->i = mipscpu->cp2dr[31].d;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2CR0:
            info->i = mipscpu->cp2cr[0].d;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2CR1:
            info->i = mipscpu->cp2cr[1].d;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2CR2:
            info->i = mipscpu->cp2cr[2].d;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2CR3:
            info->i = mipscpu->cp2cr[3].d;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2CR4:
            info->i = mipscpu->cp2cr[4].d;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2CR5:
            info->i = mipscpu->cp2cr[5].d;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2CR6:
            info->i = mipscpu->cp2cr[6].d;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2CR7:
            info->i = mipscpu->cp2cr[7].d;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2CR8:
            info->i = mipscpu->cp2cr[8].d;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2CR9:
            info->i = mipscpu->cp2cr[9].d;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2CR10:
            info->i = mipscpu->cp2cr[10].d;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2CR11:
            info->i = mipscpu->cp2cr[11].d;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2CR12:
            info->i = mipscpu->cp2cr[12].d;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2CR13:
            info->i = mipscpu->cp2cr[13].d;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2CR14:
            info->i = mipscpu->cp2cr[14].d;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2CR15:
            info->i = mipscpu->cp2cr[15].d;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2CR16:
            info->i = mipscpu->cp2cr[16].d;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2CR17:
            info->i = mipscpu->cp2cr[17].d;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2CR18:
            info->i = mipscpu->cp2cr[18].d;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2CR19:
            info->i = mipscpu->cp2cr[19].d;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2CR20:
            info->i = mipscpu->cp2cr[20].d;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2CR21:
            info->i = mipscpu->cp2cr[21].d;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2CR22:
            info->i = mipscpu->cp2cr[22].d;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2CR23:
            info->i = mipscpu->cp2cr[23].d;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2CR24:
            info->i = mipscpu->cp2cr[24].d;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2CR25:
            info->i = mipscpu->cp2cr[25].d;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2CR26:
            info->i = mipscpu->cp2cr[26].d;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2CR27:
            info->i = mipscpu->cp2cr[27].d;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2CR28:
            info->i = mipscpu->cp2cr[28].d;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2CR29:
            info->i = mipscpu->cp2cr[29].d;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2CR30:
            info->i = mipscpu->cp2cr[30].d;
            break;
        case CPUINFO_INT_REGISTER + MIPS_CP2CR31:
            info->i = mipscpu->cp2cr[31].d;
            break;
...
uint32 mips_get_cause(MIPS_CPU_CONTEXT *mipscpu) {
    return mipscpu->cp0r[CP0_CAUSE];
}

uint32 mips_get_status(MIPS_CPU_CONTEXT *mipscpu) {
    return mipscpu->cp0r[CP0_SR];
}

void mips_set_status(MIPS_CPU_CONTEXT *mipscpu, uint32 status) {
    mipscpu->cp0r[CP0_SR] = status;
}

uint32 mips_get_ePC(MIPS_CPU_CONTEXT *mipscpu) {
    return mipscpu->cp0r[CP0_EPC];
}

int mips_get_icount(MIPS_CPU_CONTEXT *mipscpu) {
    return mipscpu->mips_ICount;
}

void mips_set_icount(MIPS_CPU_CONTEXT *mipscpu, int count) {
    mipscpu->mips_ICount = count;
}
...
void psxcpu_get_info(MIPS_CPU_CONTEXT *mipscpu, UINT32 state, union cpuinfo *info)
{
	switch (state)
	{
		/* --- the following bits of info are returned as NULL-terminated strings --- */
//		case CPUINFO_STR_NAME:							strcpy(info->s = cpuintrf_temp_str(), "PSX CPU"); break;
		...
		default:
			mips_get_info(mipscpu, state, info);
			break;
	}
}
#endif
