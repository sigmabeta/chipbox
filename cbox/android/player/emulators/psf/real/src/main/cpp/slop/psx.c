...
void EMU_CALL psx_clear_state(void *state, uint8 version) {
    ...
    if (version == 2) {
        PSXSTATE->offset_to_vfs = offset;
        offset += vfs_get_state_size();
    }
    PSXSTATE->offset_to_iop = offset;
    offset += iop_get_state_size(version);
    ...
    switch (version) {
        case 1:
            hle_init_ps1(IOPSTATE);
            break;
        case 2:
            hle_init_ps2(IOPSTATE);
            break;
    }
    // Done
}
...
//
// Direct module read for the HLE PS2 loader. `state` is the PSX_STATE
// (reached from the IOP via IOPSTATE->psx_state).
//
sint32 EMU_CALL psx_hle_readfile(
        void *state, const char *path, sint32 offset, char *buffer, sint32 length
) {
    if (!(HAVE_VFS)) return -5;
    return vfs_readfile(VFSSTATE, path, offset, buffer, length);
}
...
static sint32 EMU_CALL vopen(void *vfsstate, uint8 *ram_native, uint32 ram_size, sint32 ofs) {
    ...
    sint32 i;
    for (i = 0; i < 255; i++) {
        char c = ram_native[(ofs & (ram_size - 1)) ^ (EMU_ENDIAN_XOR(3))];
        ofs++;
        if (!c) break;
        path[i] = c;
    }
...
static sint32 EMU_CALL
vread(void *vfsstate, uint8 *ram_native, uint32 ram_size, sint32 emufd, sint32 ofs, sint32 len) {
    sint32 r;
    if (emufd < 0) return -9;
    if (len < 0) return -22;
...
sint32 EMU_CALL psx_emucall(
    ...
    if (type == 0) {
        if (PSXSTATE->console_callback) {
            sint32 i;
            for (i = 0; i < arg1; i++) {
                char c = ram_native[ofs & (ram_size - 1)];
                ofs++;
    ...
    switch (type) {
        case 3:
            return vopen(VFSSTATE, ram_native, ram_size, ofs);
        case 4:
            return vclose(VFSSTATE, emufd);
        case 5:
            return vread(VFSSTATE, ram_native, ram_size, emufd, ofs, arg1);
        case 6:
            return -13; // EACCES permission denied
        case 7:
            return vlseek(VFSSTATE, emufd, arg1, arg2);
        default:
            return -5;
    }
}
