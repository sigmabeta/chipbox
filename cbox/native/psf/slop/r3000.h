...
typedef void   (EMU_CALL *r3000_store_callback_t  )(void *hwstate, uint32 a, uint32 d,
                                                    uint32 dmask);

typedef void   (EMU_CALL *r3000_advance_callback_t)(void *hwstate, uint32 cycles);

struct R3000_MEMORY_TYPE {
    uint32 mask, n;
    void *p;
};
struct R3000_MEMORY_MAP {
    uint32 x, y;
    struct R3000_MEMORY_TYPE type;
};
...
//
// Branch-delay-slot state. HE models a taken branch as a 2-tick countdown
// (slot) to a target (slot_target); there is no separate load-delay slot.
// The HLE engine needs to snapshot/restore this across IOP thread switches
// and to reason about whether a trap happened in a delay slot (plan R1).
//   R3000_REG_DELAY  = slot countdown (0 = not in a delay slot)
//   R3000_REG_DELAYV = pending branch target (valid iff slot != 0)
//
#define R3000_REG_DELAY  (68)
#define R3000_REG_DELAYV (69)
#define R3000_REG_MAX (70)
