...
//
// Direct file read for the HLE PS2 loader (reads whole modules from the
// PSF2 filesystem). Returns bytes read, or negative on error.
//
sint32 EMU_CALL vfs_readfile(void *state, const char *path, sint32 offset,
                             char *buffer, sint32 length) {
    if (!(VFSSTATE->readfile_cb)) return -5;
    return (VFSSTATE->readfile_cb)(VFSSTATE->readfile_context, path, offset,
                                   buffer, length);
}
...
sint32 EMU_CALL vfs_read(void *state, sint32 fd, char *buffer, sint32 length) {
    ...
    if (getenv("VFSLOG"))
        fprintf(stderr, "[vfs] read fd=%d \"%s\" ofs=%d len=%d -> %d\n",
                (int) fd, VFSSTATE->path[fd], (int) VFSSTATE->ofs[fd], (int) length, (int) r);
