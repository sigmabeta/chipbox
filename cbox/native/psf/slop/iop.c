...
// Global SPU-sample clock for the PSF2-HLE pump (diagnostics only).
unsigned long long g_hle_pump_sample = 0;
...
void EMU_CALL iop_clear_state(void *state, int version) {
    ...
    IOPSTATE->offset_to_map_load = offset;
    offset += sizeof(struct R3000_MEMORY_MAP) * iop_map_load_entries;
    IOPSTATE->offset_to_map_store = offset;
    offset += sizeof(struct R3000_MEMORY_MAP) * iop_map_store_entries;
    IOPSTATE->offset_to_ioptimer = offset;
    offset += ioptimer_get_state_size();
    IOPSTATE->offset_to_r3000 = offset;
    offset += r3000_get_state_size();
    IOPSTATE->offset_to_spu = offset;
    offset += spu_get_state_size(version);
    //
    // Set other variables
    //
...
//
// Direct IOP RAM base (2 MB, host-endian). Used by the HLE engine, which
// needs to read/write kernel structures directly.
//
void *EMU_CALL iop_get_ram(void *state) { return PSXRAM_BYTE_NATIVE; }

//
// The owning PSX_STATE (set by iop_execute). Used by the HLE PS2 loader
// to reach the VFS for module reads.
//
void *EMU_CALL iop_get_psx_state(void *state) { return IOPSTATE->psx_state; }

//
// Request a clean halt from outside the CPU loop (e.g. HLE). Mirrors the
// virtual-IO quit path: iop_execute returns -1, a successful track end.
//
void EMU_CALL iop_request_quit(void *state) {
    IOPSTATE->quitflag = 1;
...
** Auditing functions
*/
static EMU_INLINE void EMU_CALL
audit(struct IOP_STATE *state, uint32 address, uint32 length, uint8 t) {
...
//PSXTRACE3("audit_lw(%08X,%08X,%08X)\n",state,a,mask);
    ...
    a &= 0x1FFFFC;
    if ((mask & 0x000000FF) && (!(IOPSTATE->audit_map[a + 0]))) {
        IOPSTATE->audit_map[a + 0] = IOP_AUDIT_READ;
        IOPSTATE->audit_bytes_used++;
    }
    if ((mask & 0x0000FF00) && (!(IOPSTATE->audit_map[a + 1]))) {
        IOPSTATE->audit_map[a + 1] = IOP_AUDIT_READ;
        IOPSTATE->audit_bytes_used++;
    }
    if ((mask & 0x00FF0000) && (!(IOPSTATE->audit_map[a + 2]))) {
        IOPSTATE->audit_map[a + 2] = IOP_AUDIT_READ;
        IOPSTATE->audit_bytes_used++;
    }
    if ((mask & 0xFF000000) && (!(IOPSTATE->audit_map[a + 3]))) {
        IOPSTATE->audit_map[a + 3] = IOP_AUDIT_READ;
        IOPSTATE->audit_bytes_used++;
    }
...
//PSXTRACE3("audit_sw(state,%08X,%08X,%08X)\n",a,d,mask);
    ...
    a &= 0x1FFFFC;
    if ((mask & 0x000000FF) && (!(IOPSTATE->audit_map[a + 0])))
        IOPSTATE->audit_map[a + 0] = IOP_AUDIT_WRITE;
    if ((mask & 0x0000FF00) && (!(IOPSTATE->audit_map[a + 1])))
        IOPSTATE->audit_map[a + 1] = IOP_AUDIT_WRITE;
    if ((mask & 0x00FF0000) && (!(IOPSTATE->audit_map[a + 2])))
        IOPSTATE->audit_map[a + 2] = IOP_AUDIT_WRITE;
    if ((mask & 0xFF000000) && (!(IOPSTATE->audit_map[a + 3])))
        IOPSTATE->audit_map[a + 3] = IOP_AUDIT_WRITE;
...
void EMU_CALL iop_get_event(void *state, uint64 *time, uint32 *type, char **fmt, uint32 *arg) {
    ...
    ev = IOPSTATE->event +
         (((IOPSTATE->event_write_index + IOP_MAX_EVENTS) - IOPSTATE->event_count) %
          IOP_MAX_EVENTS);
...
static void EMU_CALL intr_update_ip(struct IOP_STATE *state) {
    // The HLE owns the CPU IRQ line (via hle psx_irq_update); HE's own
    // intr bits (vblank/sound/etc.) must not also assert it. No-op.
    (void) state;
}
...
//  uint32 i;
    ...
    */
    ...
    switch (realchan) {
        case 4: // SPU CORE0
            spu_dma(SPUSTATE, 0, PSXRAM_BYTE_NATIVE, mem_address & 0x1FFFFC, 0x1FFFFC, len,
                    is_writing);
            cycles_delay = DMA_SPU_CYCLES_PER_HALFWORD * (len / 2);
            break;
        case 7: // SPU CORE1
            spu_dma(SPUSTATE, 1, PSXRAM_BYTE_NATIVE, mem_address & 0x1FFFFC, 0x1FFFFC, len,
                    is_writing);
            cycles_delay = DMA_SPU_CYCLES_PER_HALFWORD * (len / 2);
            break;
        case 10: // SIF
...
static uint32 EMU_CALL dma_lw(struct IOP_STATE *state, uint32 core, uint32 a, uint32 mask) {
    ...
    if (chan == 7) {
        switch (a & 0xC) {
            ...
            case 0x8:
                d = 0;
                break; // sometimes this is blocked until it's 0
        }
    }
...
static uint32 EMU_CALL dma0_lw(struct IOP_STATE *state, uint32 a, uint32 mask) {
    return dma_lw(state, 0, a, mask);
}

static uint32 EMU_CALL dma1_lw(struct IOP_STATE *state, uint32 a, uint32 mask) {
    return dma_lw(state, 1, a, mask);
}
...
static void EMU_CALL dma_sw(struct IOP_STATE *state, uint32 core, uint32 a, uint32 d, uint32 mask) {
    ...
    if (chan == 7) {
        ...
        switch (a & 0xC) {
            case 0x0:
                dma->chan[chan].MADR = (dma->chan[chan].MADR & (~mask)) | (d & mask);
                break;
            case 0x4:
                dma->chan[chan].BCR = (dma->chan[chan].BCR & (~mask)) | (d & mask);
                break;
            case 0x8:
...
static void EMU_CALL dma0_sw(struct IOP_STATE *state, uint32 a, uint32 d, uint32 mask) {
    dma_sw(state, 0, a, d, mask);
}

static void EMU_CALL dma1_sw(struct IOP_STATE *state, uint32 a, uint32 d, uint32 mask) {
    dma_sw(state, 1, a, d, mask);
}
...
static void EMU_CALL flush_sound(struct IOP_STATE *state) {
    // PSF2-HLE renders the SPU2 explicitly, one sample per IOP slice, in
    // iop_execute's dedicated per-sample pump (the reference psf2_gen lockstep) so
    // a mid-slice SPU position/ENDX poll is sample-accurate. The cycle-
    // accumulator path here must NOT also render (it would double-render
    // and reintroduce the variable-cycle position phase error).
...
static void EMU_CALL iop_spu_sw(void *state, uint32 a, uint32 d, uint32 mask) {
    { static int n=0; if(getenv("SPU1LOG") && n++<40000)
        fprintf(stderr,"[spu1] sw %04X = %08X (m%08X)\n", a & 0x1FFF, d, mask); }
...
static uint32 EMU_CALL iop_spu2_lw(void *state, uint32 a, uint32 mask) {
    ...
    if (IOPSTATE->version == 2) {
        ...
        if (mask & 0x0000FFFF) d |= ((((uint32) (spu_lh(SPUSTATE, (a & (~3)) + 0))) & 0xFFFF) << 0);
        if (mask & 0xFFFF0000)
            d |= ((((uint32) (spu_lh(SPUSTATE, (a & (~3)) + 2))) & 0xFFFF) << 16);
    }
//end:
    d &= mask;
    {
        static int n = 0;
        if (getenv("SPU2LOG") && n++ < 20000)
            fprintf(stderr, "[spu2] @%llu LD %08X = %08X (mask %08X)\n",
                    g_hle_pump_sample ? g_hle_pump_sample
                                      : (unsigned long long) (IOPSTATE->odometer /
                                                              IOPSTATE->cycles_per_sample),
                    a, d, mask);
    }
...
static void EMU_CALL iop_spu2_sw(void *state, uint32 a, uint32 d, uint32 mask) {
    {
        static int n = 0;
        if (getenv("SPU2LOG") && n++ < 20000)
            fprintf(stderr, "[spu2] @%llu sw %08X = %08X (mask %08X)\n",
                    g_hle_pump_sample ? g_hle_pump_sample
                                      : (unsigned long long) (IOPSTATE->odometer /
                                                              IOPSTATE->cycles_per_sample),
                    a, d, mask);
    }
    if (getenv("ATTRLOG") && (a & 0x1FFFFFFF) == 0x1F900198 && d == 0xC0A00000) {
        static int dumped = 0;
        void *r = R3000STATE;
        uint32 pc = r3000_getreg(r, R3000_REG_PC);
        fprintf(stderr, "[attr] core0 ATTR<=C0A0 @%llu PC=%08X ra=%08X "
                        "s0=%08X s1=%08X s2=%08X a0=%08X v0=%08X\n",
                g_hle_pump_sample, pc, r3000_getreg(r, 31),
                r3000_getreg(r, 16), r3000_getreg(r, 17), r3000_getreg(r, 18),
                r3000_getreg(r, 4), r3000_getreg(r, 2));
        if (getenv("ATTRDUMP") && !dumped) {
            uint32 ra = r3000_getreg(r, 31);
            uint32 base = (ra & ~0xFu) - 0x80, k;
            dumped = 1;
            for (k = 0; k < 112; k++) {
                uint32 wa = base + k * 4;
                fprintf(stderr, "[code] %08X: %08X\n", wa,
                        IOPSTATE->ram[(wa & 0x1FFFFF) / 4]);
            }
        }
    }
...
static void EMU_CALL iop_emucall_sw(void *state, uint32 a, uint32 d, uint32 mask) {
    ...
    switch (type) {
        case 0:
            event(IOPSTATE, IOP_EVENT_VIRTUAL_IO, "Virtual console output(0x%X, 0x%X)", ofs, arg1,
                  0, 0);
            break;
        case 1:
            event(IOPSTATE, IOP_EVENT_VIRTUAL_IO, "Virtual quit", 0, 0, 0, 0);
            break;
        case 3:
            event(IOPSTATE, IOP_EVENT_VIRTUAL_IO, "Virtual open(%d, 0x%X, 0x%X, 0x%X)", emufd, ofs,
                  arg1, arg2);
            break;
        case 4:
            event(IOPSTATE, IOP_EVENT_VIRTUAL_IO, "Virtual close(%d)", emufd, 0, 0, 0);
            break;
        case 5:
            event(IOPSTATE, IOP_EVENT_VIRTUAL_IO, "Virtual read(%d, 0x%X, 0x%X)", emufd, ofs, arg1,
                  0);
            break;
        case 6:
            event(IOPSTATE, IOP_EVENT_VIRTUAL_IO, "Virtual write(%d, 0x%X, 0x%X)", emufd, ofs, arg1,
                  0);
            break;
        case 7:
            event(IOPSTATE, IOP_EVENT_VIRTUAL_IO, "Virtual lseek(%d, 0x%X, 0x%X)", emufd, arg1,
                  arg2, 0);
            break;
        default:
            event(IOPSTATE, IOP_EVENT_VIRTUAL_IO, "Virtual unknown event(0x%X, 0x%X, 0x%X, 0x%X)",
                  emufd, ofs, arg1, arg2);
            break;
    }
...
static void EMU_CALL iop_advance(void *state, uint32 elapse) {
    ...
    if (!elapse) return;
    //
    // NOTE: a handler/sync-drain must NOT freeze the audio+counter clock.
    // On real hardware the SPU2 keeps generating output and the timrman
    // timer keeps counting *while* the IOP runs an IRQ handler; audio-
    // sample time and the sequencer counters advance together, so their
    // ratio is preserved. The handler cycles legitimately advance both
    // clocks here.
    //
    // HLE drives its own root counters / SPU-DMA completion on HE's clock.
    //
    hle_advance(IOPSTATE, elapse);
    //
    // Check timers
    //
    ...
    ** Check SPU IRQ
    */
    if (elapse >= IOPSTATE->sound_cycles_until_interrupt) {
        // SPU2 IRQ (IOP_INT_SPU, bit 9). In HLE PS2 mode HE's CPU IRQ line
        // is owned by the HLE layer (intr_update_ip is gated off), so deliver
        // it to the libsd-registered irq9 handler. This is the SPU2-side
        // half of the FFXI sequencer cadence (the timrman tick is the other
        // half). The handler softcall is per-routine deduped + serialised
        // (hle_ps2.c call_irq_routine), so a fast IRQA cadence cannot flood.
...
static uint32 EMU_CALL cycles_until_next_interrupt(struct IOP_STATE *state, uint32 min) {
    ...
    // hle_slice_end -- which ticks the IOP counters and runs the FFXI
    // sequencer/audio handler synchronously -- interleaves once per output
    // sample. This reproduces the reference's psf2_gen lockstep (spu_advance(1) +
    // ps2_hw_slice per sample); without it the SPU2 renders large batches
    // with voice state frozen between coarse sequencer ticks, audible as
    // rhythmic glitching / wrong, attenuated music.
    if (IOPSTATE->version == 2 && min > 768) min = 768;
    return min;
}
...
static const struct R3000_MEMORY_MAP iop_map_load[] = {
        ...
        {0x1F800000, 0x1F800FFF, {0x000003FF, R3000_MAP_TYPE_POINTER, STATEOFS(IOP_STATE,
                                                                               scratch)}},
...
static void recompute_memory_maps(struct IOP_STATE *state) {
    ...
    //
    // The third load entry (0x1FC00000) is the BIOS-ROM region. There is
    // no BIOS image: map it to a callback that returns the sentinel so any
    // fetch/load there traps into hle_dispatch.
    //
    mapload[2].type.mask = 0x1FFFFFFF;
    mapload[2].type.n = R3000_MAP_TYPE_CALLBACK;
    mapload[2].type.p = (void *) bios_hle_lw;

    //
    // A BIOS would normally mediate the interrupt controller, root counters
    // and SPU DMA. Re-point those callback ranges at the HLE so the sound
    // driver sees the reference's RootCounter/IRQ semantics. SPU core, scratch and
    // RAM stay HE's. Identify entries by their static handler.
    //
    for (i = 0; i < (sint32) iop_map_load_entries; i++) {
        if (mapload[i].type.p == (void *) intr_lw) mapload[i].type.p = (void *) hle_intr_lw;
        if (mapload[i].type.p == (void *) timer_lw) mapload[i].type.p = (void *) hle_timer_lw;
        if (mapload[i].type.p == (void *) dma0_lw) mapload[i].type.p = (void *) hle_dma_lw;
        if (mapload[i].type.p == (void *) dma1_lw) mapload[i].type.p = (void *) hle_dma_lw;
    }
    for (i = 0; i < (sint32) iop_map_store_entries; i++) {
        if (mapstore[i].type.p == (void *) intr_sw) mapstore[i].type.p = (void *) hle_intr_sw;
        if (mapstore[i].type.p == (void *) timer_sw) mapstore[i].type.p = (void *) hle_timer_sw;
        if (mapstore[i].type.p == (void *) dma0_sw) mapstore[i].type.p = (void *) hle_dma_sw;
        if (mapstore[i].type.p == (void *) dma1_sw) mapstore[i].type.p = (void *) hle_dma_sw;
    }
...
//MessageBox(NULL,"qfset_on_execute",NULL,MB_OK);
    ...
    // ---- PSF2-HLE dedicated per-sample pump (the reference psf2_gen lockstep) ----
    // For each output sample: render exactly ONE SPU2 sample, then run a
    // fixed one-sample IOP slice. This locks the SPU2 decode/playback
    // position to the *output-sample count* (deterministic), so when the
    // FFXI/etc. engine polls an SPU2 position/ENDX register mid-slice it
    // sees a sample-accurate, hardware-consistent value -- fixing the
    // systemic ENDX/position phase error (variable IOP-cycle count between
    // the engine's clear and read was crossing sample boundaries). the reference
    // does spu_advance(1) BEFORE ps2_hw_slice, so we render the sample
    // first (with current voice state) then run the IOP. hw_sync inside
    // r3000_execute(768) ticks hle_advance->psx_hw_runcounters once per
    // sample (timer/sys_time/dma_delay), matching the reference's cadence.
    // flush_sound() is a no-op for PSF2-HLE so it cannot double-render.
    if (IOPSTATE->version == 2) {
        // the reference ps2_hw_slice per output sample:
        //   spu_advance(1); psx_hw_runcounters(); mips_execute(768/CLOCK_DIV)
        // CLOCK_DIV is 8 -> 96 IOP instructions per sample (DIVIDER==1).
        // Running the canonical 768 instr/sample (HE's native rate) made the
        // engine's SPU position polls see an 8x-wrong sample ratio; matching
        // the reference's 96 makes a mid-slice ENDX/position read sample-accurate.
        const uint32 IOP_PER_SAMPLE = 96;          /* 768 / CLOCK_DIV(8) */
        uint32 want = IOPSTATE->sound_buffer_samples_free;
        uint32 done = 0, stuck = 0;
        sint16 *bp = sound_buf;
        sint16 tmp[2];
        r = 0;
        hle_ps2_set_pump(1);
        while (done < want) {
            uint64 od0 = IOPSTATE->odometer;
            if (IOPSTATE->quitflag) {
                r = -1;
                break;
            }
            ...
            // SPU2 IRQ (libsd irq9) must fire only when the SPU2 actually
            // crosses its programmed IRQA address -- exactly as the BIOS path
            // does via cycles_until_next_interrupt/iop_advance. The pump has
            // its own loop and never calls cycles_until_next_interrupt, so
            // sound_cycles_until_interrupt stays stale and iop_advance's
            // `elapse >= sound_cycles_until_interrupt` gate fired the irq9
            // handler on essentially every hw_sync (~44k/s), waking the sound
            // thread ~once per output sample -> the sequencer raced ~7x ahead
            // of playback (correct SPU2 writes, ~7x time-compressed). Query
            // the SPU's non-destructive IRQA lookahead for the one sample we
            // are about to render; deliver the IRQ once iff the crossing
            // falls within it (cyc < 768). iop_advance's HLE delivery is
            // gated off under the pump (it owns SPU2-IRQ delivery here).
            {
                uint32 ci = spu_cycles_until_interrupt(SPUSTATE, 1);
                spu_render(SPUSTATE, bp ? bp : tmp, 1);  // the reference spu_advance(1)
                if (ci != 0xFFFFFFFFu && ci < 768) hle_ps2_spu_irq();
            }
            if (bp) bp += 2;
            done++;
            g_hle_pump_sample++;                       // global SPU-sample clock (diag)
            // the reference runs a DMA/timer completion handler (call_irq_routine)
            // SYNCHRONOUSLY the instant its delay expires inside
            // psx_hw_runcounters -- i.e. BEFORE that sample's mips_execute, so
            // libsd's main thread cannot run past a kicked transfer before its
            // completion ISR has run. Our pump_tick (== psx_hw_runcounters)
            // only *queues* the softcall; left to hle_slice_end it runs AFTER
            // this sample's 96-instr r3000_execute, during which libsd races
            // on to the next SPU2 core -> its transfer-completion finish path
            // (poll 0x344 / restore ATTR C080) lands out of order vs core
            // servicing -> notes mapped to wrong voices -> wrong pitches.
            // Drain ONLY the completion that pump_tick just raised this sample
            // (queue went empty -> non-empty), before the main thread runs --
            // exactly the reference's at-expiry timing. Don't touch the normal
            // slice_end cadence for already-pending / mid-instruction IRQs
            // (draining the whole FIFO every sample regressed correctness).
            {
                int pend0 = (int) hle_ps1_softcall_pending();
                hle_ps2_pump_tick(
                        IOPSTATE);            // root counters + the reference psx_hw_runcounters
                hle_ps2_frame_tick();                   // the reference psf2_gen vblank/frame model
                if (!pend0 && hle_ps1_softcall_pending())
                    hle_slice_end(R3000STATE, IOPSTATE);  // run the at-expiry completion now
            }
            r = r3000_execute(R3000STATE, IOP_PER_SAMPLE);
            if (r < 0) {
                r = -2;
                break;
            }
            if (IOPSTATE->odometer == od0) {
                if (++stuck > 4000000) {
                    r = -2;
                    break;
                }
        ...
        hle_ps2_set_pump(0);
        IOPSTATE->sound_buffer_samples_free = want - done;
        *sound_samples = done;
        if (r >= 0) r = (sint32) (IOPSTATE->odometer - old_odometer);
        {
            static unsigned long long iacc = 0, sacc = 0, next = 0;
            if (getenv("IPSLOG")) {
                iacc += (IOPSTATE->odometer - old_odometer);
                sacc += done;
                if (sacc >= next) {
                    next = sacc + 44100;
                    fprintf(stderr, "[ips] sample=%llu  IOP-instr/sample=%.1f  (ref=96)\n",
                            sacc, sacc ? (double) iacc / (double) sacc : 0.0);
                }
            }
        }
        return r;
    ...
    */
    {
        uint64 wd_last = ~(uint64) 0;
        uint32 wd_stuck = 0;
        for (;;) {
            uint32 diff, ci;
            if (IOPSTATE->odometer >= target_odometer) break;

            if (IOPSTATE->odometer == wd_last) {
                if (++wd_stuck > 2000000) {
                    if (getenv("HLE_IOP"))
                        fprintf(stderr, "[iop] WATCHDOG: odometer stuck at %llu pc=%08X "
                                        "diff path -- bailing\n",
                                (unsigned long long) IOPSTATE->odometer,
                                r3000_getreg(R3000STATE, R3000_REG_PC));
                    r = -2;
                    break;
                }
            } else {
                wd_last = IOPSTATE->odometer;
                wd_stuck = 0;
            }
...
uint32 EMU_CALL iop_getword(void *state, uint32 a) {
    a &= 0x1FFFFFFC;
    if (a < 0x00800000) {
        return (*((uint32 *) (PSXRAM_BYTE_NATIVE + (a & 0x1FFFFC))));
    } else if (a >= 0x1FC00000) {
        return R3000_HLE_SENTINEL;  // no BIOS image
    } else {
        return 0;
    }
}
