 ...
 * Android impls wrap Hilt's per-`NavBackStackEntry` `hiltViewModel<T>()`; the JVM impl
 * dispatches to its plain-Dagger `JvmChipboxComponent`. The provider's lifetime + scoping is
 * its own concern — `chipboxViewModel()` just asks for a `T` and trusts whatever lifecycle
 * policy the platform's provider follows.
 */
