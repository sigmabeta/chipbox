...
/**
 * JS `WithPerScreenViewModelStore` — Voyager's non-Android
 * `DefaultNavigatorScreenLifecycleProvider` returns `emptyList()`, so without this every
 * Screen would share the root `ViewModelStoreOwner` and `metroViewModel<VM>()` would return
 * the same instance across pushes.
 *
