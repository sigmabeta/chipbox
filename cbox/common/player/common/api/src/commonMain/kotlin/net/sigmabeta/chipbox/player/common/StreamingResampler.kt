 ...
 * interpolation [kernel]; this base owns the phase advance, the bounds that decide which output
 * frames can be produced from the current input buffer, and the per-channel history carried across
 * [process] calls so a kernel that reaches past a buffer edge stays continuous.
 *
 * Phase is kept as an **exact rational**: an integer input index [inPos] plus a fraction
 * [phaseNum]`/`[outputRate]. Each output advances the input position by `inputRate/outputRate` via
 * integer arithmetic, so there is no floating-point drift — the pitch can't wander over a long
 * track, and feeding a stream split across buffers is bit-identical to feeding it whole. A `Double`
 * is formed only to evaluate the kernel between two samples.
 *
...
abstract class StreamingResampler(
    final override val inputRate: Int,
    final override val outputRate: Int,
    ...
    final override fun maxOutputFrames(inputFrames: Int): Int =
    ...
    final override fun process(input: ShortArray, inputFrames: Int, output: ShortArray): Int {
        var outFrames = 0
        while (true) {
