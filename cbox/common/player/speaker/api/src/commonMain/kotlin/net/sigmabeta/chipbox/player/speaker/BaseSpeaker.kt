...
/**
 * Production base class for [Speaker]. Owns the consume loop, the [VolumeProcessor] applied to
 * every buffer, the normalisation gain bookkeeping, and the buffer recycling. Subclasses only
 * implement [onAudioReceived] (where to write each buffer) and [teardown] (release the sink);
 * sinks include Android's `AudioTrack`
 * ([net.sigmabeta.chipbox.player.speaker.real.RealSpeaker]), a WAV file
 * ([net.sigmabeta.chipbox.player.speaker.file.FileSpeaker]), or stdout
 * ([net.sigmabeta.chipbox.player.speaker.text.TextSpeaker]).
 *
 * The interface itself is in [Speaker] — tests can implement that directly to avoid dragging a
 * [ConsumerBufferManager] into their fixtures.
 *
 * ### Threading
...
abstract class BaseSpeaker(
    ...
    override fun setDucked(ducked: Boolean) = volumeProcessor.setDucked(ducked)
    ...
    override fun clearVolumeModification(key: String) = volumeProcessor.clearModification(key)
    ...
    override fun events(): SharedFlow<SpeakerEvent> = eventSink.asSharedFlow()
    ...
    override fun debugInfo(): StateFlow<SpeakerDebugInfo> = debugInfoMutable.asStateFlow()
     ...
     */
    override suspend fun seek() = drainAndRestart(targetTrackId = null, label = "seek")
