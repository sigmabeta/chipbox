 ...
 * (absent when the matched game has no cover on IGDB), and when the match was made. It sits next to
 * the downloaded `<Title>.jpg` so anyone browsing the folder can see — and double-check — where the
 * cover came from, and a later "Get cover art" run can [read] it back to skip the IGDB query for a
 * game whose folder already records its match.
 *
 * A descriptor counts as a usable record only while its "Matched" timestamp is within
 * [COVER_ART_TTL_DAYS] — the same window the [CoverArtCache] uses — so a long-stale file is re-queried
 * rather than trusted forever (letting covers IGDB has since added get picked up). The timestamp is
 * therefore "when the match was last confirmed against IGDB": [writeInto] refreshes it only on a fresh
 * lookup, and otherwise keeps the existing one so re-confirming from the cache or this file doesn't
 * churn it.
 *
...
class IgdbLinkFile(private val fileSystem: FileSystem) {
    /**
     ...
     */
    fun read(folders: List<Path>, now: Instant = Clock.System.now()): CoverLookup? =
        folders.firstNotNullOfOrNull { readOne(it / FILE_NAME, now) }

    /**
     * Writes the descriptor into each of [folders]. [refreshDate] stamps it with [now] (use it when
     * [match] came from a fresh IGDB lookup); otherwise the existing timestamp is kept for an
     * unchanged match so re-confirming from the cache or the file itself doesn't move it.
     */
    fun writeInto(
        folders: List<Path>,
        title: String,
        ...
        for (folder in folders) {
            runCatching { writeOne(folder / FILE_NAME, body, refreshDate, now) }
        }
    }
    ...
    private fun readOne(file: Path, now: Instant): CoverLookup? {
        val lines = readTextOrNull(file)
            ?.lines()
            ?.takeUnless { isStale(it, now) }
            ?: return null
    ...
    private fun writeOne(file: Path, body: List<String>, refreshDate: Boolean, now: Instant) {
        val kept = if (refreshDate) null else unchangedStamp(file, body)
        val content = (body + line(MATCHED_LABEL, kept ?: now.toString())).joinToString("\n") + "\n"
        if (readTextOrNull(file) != content) {
            file.parent?.let { fileSystem.createDirectories(it) }
            fileSystem.write(file) { writeUtf8(content) }
        }
    }

    // True when the descriptor's "Matched" timestamp is missing, unparseable, or older than the TTL —
    // i.e. the recorded match shouldn't be trusted without re-checking IGDB.
    private fun isStale(lines: List<String>, now: Instant): Boolean {
        val matched = value(lines, MATCHED_LABEL)?.let { runCatching { Instant.parse(it) }.getOrNull() }
        return matched == null || (now - matched) > COVER_ART_TTL_DAYS.days
    }

    // If [file] already describes this exact match, returns its recorded timestamp so it isn't bumped
    // to now; otherwise null, meaning the descriptor is new or has changed and should be re-stamped.
    private fun unchangedStamp(file: Path, body: List<String>): String? {
        val lines = readTextOrNull(file)?.trimEnd('\n')?.lines() ?: return null
    ...
    private fun readTextOrNull(file: Path): String? {
        if (fileSystem.metadataOrNull(file)?.isRegularFile != true) return null
        return runCatching { fileSystem.read(file) { readUtf8() } }.getOrNull()
    }
