...
void SPUwriteRegister(u32 reg, u16 val) {
    ...
    switch (r) {
        ...
        case H_SPUReverbAddr:
            if (val == 0xFFFF || val <= 0x200) { rvb.StartAddr = rvb.CurrAddr = 0; }
            else {
...
static void SoundOn(int start, int end, u16 val)     // SOUND ON PSX COMAND
{
    ...
    {
        if ((val & 1) &&
            s_chan[ch].pStart)                    // mmm... start has to be set before key on !?!
        {
...
static void SetVolumeLR(int right, u8 ch, s16 vol)            // LEFT VOLUME
{
    ...
    if (vol & 0x8000)                                        // sweep?
    {
        ...
        vol += vol / (2 *
                      sInc);                                  // -> HACK: we don't sweep right now, so we just raise/lower the volume by the half!
        vol *= 128;
        vol &= 0x3fff;
        //puts("Sweep");
    } else                                                  // no sweep:
