...
static void InitADSR(void)                                    // INIT ADSR
{
    ...
    memset(RateTable, 0, sizeof(u32) *
                         160);        // build the rate table according to Neill's rules (see at bottom of file)
    ...
    for (i = 32; i <
                 160; i++)                                   // we start at pos 32 with the real values... everything before is 0
    {
        if (r < 0x3FFFFFFF) {
            r += rs;
            rd++;
...
static INLINE int MixADSR(int ch)                             // MIX ADSR
{
    ...
    {                                                    // do release
        if (s_chan[ch].ADSRX.ReleaseModeExp) {
            s_chan[ch].ADSRX.EnvelopeVol -= RateTable[(4 * (s_chan[ch].ADSRX.ReleaseRate ^ 0x1F)) -
                                                      0x18 + 32 + sexytable[
                                                              (s_chan[ch].ADSRX.EnvelopeVol >> 28) &
                                                              0x7]];
        } else {
            s_chan[ch].ADSRX.EnvelopeVol -= RateTable[(4 * (s_chan[ch].ADSRX.ReleaseRate ^ 0x1F)) -
                                                      0x0C + 32];
    ...
    } else                                                  // not stopped yet?
    {
        if (s_chan[ch].ADSRX.State == 0)                       // -> attack
        {
            if (s_chan[ch].ADSRX.AttackModeExp) {
                if (s_chan[ch].ADSRX.EnvelopeVol < 0x60000000)
                    s_chan[ch].ADSRX.EnvelopeVol += RateTable[(s_chan[ch].ADSRX.AttackRate ^ 0x7F) -
                                                              0x10 + 32];
                else
                    s_chan[ch].ADSRX.EnvelopeVol += RateTable[(s_chan[ch].ADSRX.AttackRate ^ 0x7F) -
                                                              0x18 + 32];
        ...
        {
            s_chan[ch].ADSRX.EnvelopeVol -= RateTable[(4 * (s_chan[ch].ADSRX.DecayRate ^ 0x1F)) -
                                                      0x18 + 32 + sexytable[
                                                              (s_chan[ch].ADSRX.EnvelopeVol >> 28) &
                                                              0x7]];
        ...
        if (s_chan[ch].ADSRX.State == 2)                       // -> sustain
        {
            if (s_chan[ch].ADSRX.SustainIncrease) {
                ...
                if (s_chan[ch].ADSRX.SustainModeExp)
                    s_chan[ch].ADSRX.EnvelopeVol -= RateTable[
                            ((s_chan[ch].ADSRX.SustainRate ^ 0x7F)) - 0x1B + 32 +
                            sexytable[(s_chan[ch].ADSRX.EnvelopeVol >> 28) & 0x7]];
                else
                    s_chan[ch].ADSRX.EnvelopeVol -= RateTable[
