...
static INLINE s64
g_buffer(int iOff)                          // get_buffer content helper: takes care about wraps
{
    s16 *p = (s16 *) spuMem;
...
static INLINE void s_buffer(int iOff,
...
static INLINE void s_buffer1(int iOff,
                             int iVal)                // set_buffer (+1 sample) content helper: takes care about wraps and clipping
{
    s16 *p = (s16 *) spuMem;
...
static INLINE void MixREVERBLeftRight(s32 *oleft, s32 *oright, s32 inleft, s32 inright) {
    ...
    if (dbpos &
        1)                                          // we work on every second left value: downsample to 22 khz
    {
        if (spuCtrl & 0x80)                                  // -> reverb on? oki
        {
            ...
            INPUT_SAMPLE_R >>= (16 - 8);
            {
                const s64 IIR_INPUT_A0 = ((g_buffer(rvb.IIR_SRC_A0) * rvb.IIR_COEF) >> 15) +
                                         ((INPUT_SAMPLE_L * rvb.IN_COEF_L) >> 15);
                const s64 IIR_INPUT_A1 = ((g_buffer(rvb.IIR_SRC_A1) * rvb.IIR_COEF) >> 15) +
                                         ((INPUT_SAMPLE_R * rvb.IN_COEF_R) >> 15);
                const s64 IIR_INPUT_B0 = ((g_buffer(rvb.IIR_SRC_B0) * rvb.IIR_COEF) >> 15) +
                                         ((INPUT_SAMPLE_L * rvb.IN_COEF_L) >> 15);
                const s64 IIR_INPUT_B1 = ((g_buffer(rvb.IIR_SRC_B1) * rvb.IIR_COEF) >> 15) +
                                         ((INPUT_SAMPLE_R * rvb.IN_COEF_R) >> 15);
                const s64 IIR_A0 = ((IIR_INPUT_A0 * rvb.IIR_ALPHA) >> 15) +
                                   ((g_buffer(rvb.IIR_DEST_A0) * (32768L - rvb.IIR_ALPHA)) >> 15);
                const s64 IIR_A1 = ((IIR_INPUT_A1 * rvb.IIR_ALPHA) >> 15) +
                                   ((g_buffer(rvb.IIR_DEST_A1) * (32768L - rvb.IIR_ALPHA)) >> 15);
                const s64 IIR_B0 = ((IIR_INPUT_B0 * rvb.IIR_ALPHA) >> 15) +
                                   ((g_buffer(rvb.IIR_DEST_B0) * (32768L - rvb.IIR_ALPHA)) >> 15);
                const s64 IIR_B1 = ((IIR_INPUT_B1 * rvb.IIR_ALPHA) >> 15) +
                                   ((g_buffer(rvb.IIR_DEST_B1) * (32768L - rvb.IIR_ALPHA)) >> 15);
                ...
                s_buffer(rvb.MIX_DEST_B0, ((rvb.FB_ALPHA * ACC0) >> 15) -
                                          ((FB_A0 * (int) (rvb.FB_ALPHA ^ 0xFFFF8000)) >> 15) -
                                          ((FB_B0 * rvb.FB_X) >> 15));
                s_buffer(rvb.MIX_DEST_B1, ((rvb.FB_ALPHA * ACC1) >> 15) -
                                          ((FB_A1 * (int) (rvb.FB_ALPHA ^ 0xFFFF8000)) >> 15) -
                                          ((FB_B1 * rvb.FB_X) >> 15));
