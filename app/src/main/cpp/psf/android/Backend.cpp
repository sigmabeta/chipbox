#include <stdio.h>
#include "net_sigmabeta_chipbox_backend_psf_BackendImpl.h"
#include <fstream>
...
void JNICALL Java_net_sigmabeta_chipbox_backend_psf_BackendImpl_readNextSamples
...
//                            "Done copying from SexyPSF. Copied %lu bytes",
...
int64_t generateAudio(uint64_t bytes_requested) {
    uint64_t internal_buffer_size = 0;
    uint8_t *sexy_buffer = get_sound_buffer_ptr();

    while (internal_buffer_size < bytes_requested) {
        if (!is_cpu_open()) {
            return -1;
        }
        ...
        uint64_t temp_size = get_sound_buffer_size();
        if (temp_size > 0) {
            fifo_put(&g_fifo_from_lib, sexy_buffer, temp_size);
            reset_sound_buffer_size();
            internal_buffer_size += temp_size;
        }
    }
    return internal_buffer_size;
}

jlong JNICALL Java_net_sigmabeta_chipbox_backend_psf_BackendImpl_getMillisPlayed
        (JNIEnv *env, jobject) {
...
JNIEXPORT jstring JNICALL Java_net_sigmabeta_chipbox_backend_psf_BackendImpl_seek
    ...
    if (target_sample_count_track <= g_played_sample_count_track) {
        teardownInternal();
        loadFileInternal(g_filename_c_str, g_sample_rate_device, g_buffer_length_shorts_device, g_track_length_ms);
    }
...
JNIEXPORT void JNICALL Java_net_sigmabeta_chipbox_backend_psf_BackendImpl_setTempo
    ...
    set_time_scaling_factor(tempo_relative);
}
...
JNIEXPORT void JNICALL Java_net_sigmabeta_chipbox_backend_psf_BackendImpl_muteVoice
    ...
    int *muted_channels = get_muted_channel_array();
    if (enabled) {
        muted_channels[channel_number] = 1;
    } else {
        muted_channels[channel_number] = 0;
    }
}
...
void loadFileInternal(const char *filename_c_str, jint rate, jlong buffer_size,
    ...
    PSFINFO *psf_info = sexy_load((char *) g_filename_c_str);
    ...
    free(psf_info);
    g_initialized = true;
    // PSX samples at 44100 always.
    g_sample_rate_track = 44100;
