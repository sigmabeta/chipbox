...
double g_time_scaling_factor = 1.0;
...
static void psxRcntUpd(u32 index) {
    ...
    if (((!(psxCounters[index].mode & 1)) || (index != 2)) &&
        ...
        if (psxCounters[index].mode & 0x10) { // Interrupt on target
            psxCounters[index].Cycle = ((psxCounters[index].target - psxCounters[index].count) *
                                        psxCounters[index].rate) / BIAS;
        } else { // Interrupt on 0xffff
            psxCounters[index].Cycle =
                    ((0xffff - psxCounters[index].count) * psxCounters[index].rate) / BIAS;
        }
...
int CounterSPURun(void) {
    ...
    if (cycles >= 16) {
        if (!SPUasync(cycles / g_time_scaling_factor)) {
            return (0);
        }
        last = psxRegs.cycle;
    }
        ...
        switch (value & 0x300) {
            case 0x100:
                psxCounters[index].rate =
                        ((psxCounters[3].rate /** BIAS*/) / 386) / 262; // seems ok
                break;
            default:
...
//	if ((!(psxCounters[index].mode & 1)) || (index!=2)) {
    if (psxCounters[index].mode & 0x08) { // Wrap at target
        ...
        //} else {
        ret = (psxCounters[index].count +
               BIAS * ((psxRegs.cycle - psxCounters[index].sCycle) / psxCounters[index].rate)) &
              0xffff;
        //}
    } else { // Wrap at 0xffff
        ret = (psxCounters[index].count + BIAS * (psxRegs.cycle / psxCounters[index].rate)) &
              0xffff;
...
void set_time_scaling_factor(double factor) {
    g_time_scaling_factor = factor;
}