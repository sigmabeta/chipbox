...
void FreeGD3Tag(GD3_TAG *TagData) {
    ...
    free(TagData->strCreator);
    TagData->strCreator = NULL;
    free(TagData->strNotes);
    TagData->strNotes = NULL;
            ...
            for (CurCSet = 0x00; CurCSet < 0x02; CurCSet++) {
                ...
                for (CurChip = 0x00; CurChip < CHIP_COUNT; CurChip++, CAA++) {
                    if (CAA->ChipType == 0xFF)    // chip unused
                        ...
                        if (FileMode == 0x01) {
                            chip_reg_write(0x09, CurCSet, 0x00, 0x01,
                                           0x20);    // Enable Waveform Select
                            chip_reg_write(0x09, CurCSet, 0x00, 0xBD,
                                           0xC0);    // Disable Rhythm Mode
                        }
                        ...
                        if (FileMode >= 0x01) {
                            ...
                            chip_reg_write(0x0C, CurCSet, 0x00, 0xBD,
                                           0xC0);    // Disable Rhythm Mode
        ...
        case 0x10:    // Set Muting Mask
            for (CurCSet = 0x00; CurCSet < 0x02; CurCSet++) {
                ...
                for (CurChip = 0x00; CurChip < CHIP_COUNT; CurChip++, CAA++) {
                    ...
                    else if (CAA->ChipType == 0x11);    // PWM - nothing to mute
                    else if (CAA->ChipType == 0x12 && !UseFM)
                    ...
                    else if (CAA->ChipType == 0x16);    // UPD7759 - nothing to mute
                    else if (CAA->ChipType == 0x17);    // OKIM6258 - nothing to mute
                    else if (CAA->ChipType == 0x18)
            ...
            switch (Command) {
                            ...
                            switch (TempByt) {
                                case 0x80:    // SegaPCM ROM
                                    if (!CHIP_CHECK(SegaPCM))
                                        break;
                                    sega_pcm_write_rom(CurChip, ROMSize, DataStart, DataLen,
                                                       ROMData);
                                    break;
                                case 0x81:    // YM2608 DELTA-T ROM Image
                                    if (!CHIP_CHECK(YM2608))
                                        break;
                                    ym2608_write_data_pcmrom(CurChip, 0x02, ROMSize, DataStart,
                                                             DataLen,
                                                             ROMData);
                                    break;
                                ...
                                case 0x84:    // YMF278B ROM Image
                                    if (!CHIP_CHECK(YMF278B))
                                        break;
                                    ymf278b_write_rom(CurChip, ROMSize, DataStart, DataLen,
                                                      ROMData);
                                    break;
                                ...
                                case 0x86:    // YMZ280B ROM Image
                                    if (!CHIP_CHECK(YMZ280B))
                                        break;
                                    ymz280b_write_rom(CurChip, ROMSize, DataStart, DataLen,
                                                      ROMData);
                                    break;
                                ...
                                case 0x89:    // MultiPCM ROM Image
                                    if (!CHIP_CHECK(MultiPCM))
                                        break;
                                    multipcm_write_rom(CurChip, ROMSize, DataStart, DataLen,
                                                       ROMData);
                                    break;
                                case 0x8A:    // UPD7759 ROM Image
                                    if (!CHIP_CHECK(UPD7759))
                                        break;
                                    upd7759_write_rom(CurChip, ROMSize, DataStart, DataLen,
                                                      ROMData);
                                    break;
                                case 0x8B:    // OKIM6295 ROM Image
                                    if (!CHIP_CHECK(OKIM6295))
                                        break;
                                    okim6295_write_rom(CurChip, ROMSize, DataStart, DataLen,
                                                       ROMData);
                                    break;
                                case 0x8C:    // K054539 ROM Image
                                    if (!CHIP_CHECK(K054539))
                                        break;
                                    k054539_write_rom(CurChip, ROMSize, DataStart, DataLen,
                                                      ROMData);
                                    break;
                                ...
                                case 0x8E:    // K053260 ROM Image
                                    if (!CHIP_CHECK(K053260))
                                        break;
                                    k053260_write_rom(CurChip, ROMSize, DataStart, DataLen,
                                                      ROMData);
                                    break;
                                ...
                                case 0x93:    // GA20 ROM Image
                                    if (!CHIP_CHECK(GA20))
                                        break;
                                    iremga20_write_rom(CurChip, ROMSize, DataStart, DataLen,
                                                       ROMData);
                                    break;
                ...
                case 0xD0:    // YMF278B write
                    if (CHIP_CHECK(YMF278B)) {
                        ...
                        chip_reg_write(0x0D, CurChip, VGMPnt[0x01] & 0x7F, VGMPnt[0x02],
                                       VGMPnt[0x03]);
                    }
                    VGMPos += 0x04;
                    break;
                case 0xD1:    // YMF271 write
                    if (CHIP_CHECK(YMF271)) {
                        CurChip = (VGMPnt[0x01] & 0x80) >> 7;
                        chip_reg_write(0x0E, CurChip, VGMPnt[0x01] & 0x7F, VGMPnt[0x02],
                                       VGMPnt[0x03]);
                    }
                    VGMPos += 0x04;
                    break;
        ...
        switch (CAA->Resampler) {
            ...
            case 0x03:    // Downsampling
                ...
                for (OutPos = 0x00; OutPos < Length; OutPos++) {
                    ...
                    InPos = InPosNext;
                    InPosNext = InBase +
                                (UINT32) (FIXPNT_FACT * (OutPos + 1) * ChipSmpRate / SampleRate);
