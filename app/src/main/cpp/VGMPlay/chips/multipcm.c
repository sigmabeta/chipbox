...
static const double BaseTimes[64] = {0, 0, 0, 0, 6222.95, 4978.37, 4148.66, 3556.01, 3111.47,
                                     2489.21, 2074.33, 1778.00, 1555.74, 1244.63, 1037.19, 889.02,
                                     777.87, 622.31, 518.59, 444.54, 388.93, 311.16, 259.32, 222.27,
                                     194.47, 155.60, 129.66, 111.16, 97.23, 77.82, 64.85, 55.60,
                                     48.62, 38.91, 32.43, 27.80, 24.31, 19.46, 16.24, 13.92, 12.15,
                                     9.75, 8.12, 6.98, 6.08, 4.90, 4.08, 3.49,
                                     3.04, 2.49, 2.13, 1.90, 1.72, 1.41, 1.18, 1.04, 0.91, 0.73,
                                     0.59, 0.50, 0.45, 0.45, 0.45, 0.45};
...
static const float LFOFreq[8] = {0.168f, 2.019f, 3.196f, 4.206f, 5.215f, 5.888f, 6.224f,
                                 7.066f};    //Hz;
static const float PSCALE[8] = {0.0f, 3.378f, 5.065f, 6.750f, 10.114f, 20.170f, 40.180f,
                                79.307f};    //cents
static const float ASCALE[8] = {0.0f, 0.4f, 0.8f, 1.5f, 3.0f, 6.0f, 12.0f,
                                24.0f};                    //DB
...
static void
LFO_ComputeStep(MultiPCM *ptChip, struct _LFO *LFO, UINT32 LFOF, UINT32 LFOS, int ALFO) {
...
static void WriteSlot(MultiPCM *ptChip, struct _SLOT *slot, int reg, unsigned char data) {
    ...
    switch (reg) {
        ...
        {
            if (data) {
                LFO_ComputeStep(ptChip, &(slot->PLFO), (slot->Regs[6] >> 3) & 7, slot->Regs[6] & 7,
                                0);
                LFO_ComputeStep(ptChip, &(slot->ALFO), (slot->Regs[6] >> 3) & 7, slot->Regs[7] & 7,
                                1);
            }
        }
            break;
    ...
    for (i = 0; i < samples; ++i) {
        ...
        for (sl = 0; sl < 28; ++sl) {
            ...
            if (slot->Playing && !slot->Muted) {
                ...
                signed int csample = (signed short) (
                        ptChip->ROM[(slot->Base + adr) & ptChip->ROMMask] << 8);
...
int device_start_multipcm(UINT8 ChipID, int clock) {
    ...
    for (i = 0; i < 0x40; ++i) {
        ...
        ptChip->DRStep[i] =
                (float) (0x400 << EG_SHIFT) / (BaseTimes[i] * AR2DR * 44100.0 / (1000.0));
    }
