...
static void generate_dummy(es5506_state *chip, es5506_voice *voice, UINT16 *base, INT32 *lbuffer,
                           INT32 *rbuffer, int samples) {
...
static void
generate_ulaw(es5506_state *chip, es5506_voice *voice, UINT16 *base, INT32 *lbuffer, INT32 *rbuffer,
              int samples) {
    UINT32 freqcount = voice->freqcount;
    UINT32 accum = voice->accum & voice->accum_mask;
...
static void
generate_pcm(es5506_state *chip, es5506_voice *voice, UINT16 *base, INT32 *lbuffer, INT32 *rbuffer,
             int samples) {
    UINT32 freqcount = voice->freqcount;
    UINT32 accum = voice->accum & voice->accum_mask;
    INT32 lvol = chip->volume_lookup[voice->lvol >> 4];
...
void device_stop_es5506(UINT8 ChipID) {
    ...
    chip->ulaw_lookup = NULL;
    free(chip->volume_lookup);
    chip->volume_lookup = NULL;
    free(chip->scratch);
    chip->scratch = NULL;
...
INLINE void
es5506_reg_write_low(es5506_state *chip, es5506_voice *voice, offs_t offset, UINT32 data) {
    switch (offset) {
        case 0x00 / 8:    /* CR */
...
INLINE void
es5506_reg_write_high(es5506_state *chip, es5506_voice *voice, offs_t offset, UINT32 data) {
    switch (offset) {
        case 0x00 / 8:    /* CR */
            voice->control = data & 0xffff;
            //if (LOG_COMMANDS && eslog)
...
INLINE void
es5506_reg_write_test(es5506_state *chip, es5506_voice *voice, offs_t offset, UINT32 data) {
    switch (offset) {
...
INLINE void
es5505_reg_write_low(es5506_state *chip, es5506_voice *voice, offs_t offset, UINT16 data) {
...
#else
                voice->control &= ~(CONTROL_STOPMASK | CONTROL_BS0 | CONTROL_LOOPMASK |
                                    CONTROL_IRQE | CONTROL_DIR | CONTROL_IRQ);
#endif
                voice->control |= (data & (CONTROL_STOPMASK | CONTROL_LOOPMASK | CONTROL_IRQE |
                                           CONTROL_DIR | CONTROL_IRQ)) |
                                  ((data << 12) & CONTROL_BS0);
            }
...
INLINE void
es5505_reg_write_high(es5506_state *chip, es5506_voice *voice, offs_t offset, UINT16 data) {
    //running_machine &machine = chip->device->machine();
    UINT8 byteOfs;
...
INLINE void
es5505_reg_write_test(es5506_state *chip, es5506_voice *voice, offs_t offset, UINT16 data) {
    UINT8 byteOfs;
...
INLINE UINT16 es5505_reg_read_low(es5506_state *chip, es5506_voice *voice, offs_t offset) {
    ...
    switch (offset) {
        case 0x00:    /* CR */
            result = (voice->control &
                      (CONTROL_STOPMASK | CONTROL_LOOPMASK | CONTROL_IRQE | CONTROL_DIR |
                       CONTROL_IRQ)) |
    ...
    switch (offset) {
        ...
        case 0x06:    /* O1(n-1) */
            ...
            if ((voice->control & CONTROL_STOPMASK) && chip->region_base[voice->control >> 14]) {
                voice->o1n1 = chip->region_base[voice->control >> 14][voice->exbank +
                                                                      (voice->accum >> 11)];
                logerror("%02x %08x ==> %08x\n", voice->o1n1, voice->control >> 14, voice->exbank +
                                                                                    (voice->accum
                                                                                            >> 11));
            }
            result = voice->o1n1;
