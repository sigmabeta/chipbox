...
void SN76496Update(void *chip, stream_sample_t **outputs, int samples) {
    ...
    while (samples > 0) {
        ...
        if (R->Count[3] <= 0) {
            ...
            if (((R->RNG & R->WhitenoiseTap1) ? 1 : 0) ^
                ((((R->RNG & R->WhitenoiseTap2) ? 1 : 0)) * (NOISEMODE))) {
                R->RNG >>= 1;
                R->RNG |= R->FeedbackMask;
            } else {
                R->RNG >>= 1;
...
static int
generic_start(sn76496_state *chip, int clock, int feedbackmask, int noisetap1, int noisetap2,
