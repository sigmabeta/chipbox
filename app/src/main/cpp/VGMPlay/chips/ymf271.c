...
static const double pow_table[16] = {128, 256, 512, 1024, 2048, 4096, 8192, 16384, 0.5, 1, 2, 4, 8,
                                     16, 32, 64};
...
static const double channel_attenuation_table[16] =
        {
                0.0, 2.5, 6.0, 8.5, 12.0, 14.5, 18.1, 20.6, 24.1, 26.6, 30.1, 32.6, 36.1, 96.1,
                96.1, 96.1
        };
...
static void init_envelope(YMF271Chip *chip, YMF271Slot *slot) {
    ...
    slot->env_attack_step = (rate < 4) ? 0 : (int) (((double) (255 - 0) / chip->lut_ar[rate]) *
                                                    65536.0);

    // init decay1 state
    rate = get_keyscaled_rate(slot->decay1rate * 2, keycode, slot->keyscale);
    slot->env_decay1_step = (rate < 4) ? 0 : (int) (
            ((double) (255 - decay_level) / chip->lut_dc[rate]) * 65536.0);

    // init decay2 state
    rate = get_keyscaled_rate(slot->decay2rate * 2, keycode, slot->keyscale);
    slot->env_decay2_step = (rate < 4) ? 0 : (int) (((double) (255 - 0) / chip->lut_dc[rate]) *
                                                    65536.0);

    // init release state
    rate = get_keyscaled_rate(slot->relrate * 4, keycode, slot->keyscale);
    slot->env_release_step = (rate < 4) ? 0 : (int) (((double) (255 - 0) / chip->lut_ar[rate]) *
                                                     65536.0);
...
static void init_lfo(YMF271Chip *chip, YMF271Slot *slot) {
    ...
    slot->lfo_step = (int) ((((double) LFO_LENGTH * chip->lut_lfo[slot->lfoFreq]) / 44100.0) *
                            256.0);
}
...
INLINE void update_lfo(YMF271Chip *chip, YMF271Slot *slot) {
    ...
    slot->lfo_amplitude = chip->lut_alfo[slot->lfowave][(slot->lfo_phase >> LFO_SHIFT) &
                                                        (LFO_LENGTH - 1)];
    slot->lfo_phasemod = chip->lut_plfo[slot->lfowave][slot->pms][(slot->lfo_phase >> LFO_SHIFT) &
                                                                  (LFO_LENGTH - 1)];
...
INLINE int calculate_slot_volume(YMF271Chip *chip, YMF271Slot *slot) {
    ...
    switch (slot->ams) {
        case 0:
            ...
            break;    // 0dB
        case 1:
            lfo_volume = 65536 - ((slot->lfo_amplitude * 33124) >> 16);
            break;    // 5.90625dB
        case 2:
            lfo_volume = 65536 - ((slot->lfo_amplitude * 16742) >> 16);
            break;    // 11.8125dB
        case 3:
            lfo_volume = 65536 - ((slot->lfo_amplitude * 4277) >> 16);
            break;    // 23.625dB
    }

    env_volume =
            (chip->lut_env_volume[255 - (slot->volume >> ENV_VOLUME_SHIFT)] * lfo_volume) >> 16;
...
static void update_pcm(YMF271Chip *chip, int slotnum, INT32 *mixp, int length) {
    ...
    for (i = 0; i < length; i++) {
        ...
        if ((slot->stepptr >> 16) > slot->endaddr) {
            slot->stepptr = slot->stepptr - ((UINT64) slot->endaddr << 16) +
                            ((UINT64) slot->loopaddr << 16);
            if ((slot->stepptr >> 16) > slot->endaddr) {
                // overflow
                slot->stepptr &= 0xffff;
        ...
        if (slot->bits == 8) {
            ...
            if (slot->stepptr & 0x10000)
                sample = ymf271_read_memory(chip, slot->startaddr + (slot->stepptr >> 17) * 3 + 2)
                                 << 8 |
                         ((ymf271_read_memory(chip, slot->startaddr + (slot->stepptr >> 17) * 3 + 1)
                                 << 4) & 0xf0);
            else
                sample =
                        ymf271_read_memory(chip, slot->startaddr + (slot->stepptr >> 17) * 3) << 8 |
                        (ymf271_read_memory(chip, slot->startaddr + (slot->stepptr >> 17) * 3 + 1) &
                         0xf0);
        }
                    ...
                    for (i = 0; i < samples; i++) {
                        ...
                        switch (chip->slots[slot1].algorithm) {
                            ...
                            case 2:
                                ...
                                phase_mod2 = calculate_op(chip, slot2,
                                                          (phase_mod1 + phase_mod3) / 1);
                                output4 = calculate_op(chip, slot4, phase_mod2);
                                break;
                        ...
                        *mixp++ +=
                                ((output1 * chip->lut_attenuation[chip->slots[slot1].ch0_level]) +
                                 ...
                                 (output4 * chip->lut_attenuation[chip->slots[slot4].ch0_level]))
                                        >> 16;
                        *mixp++ +=
                                ((output1 * chip->lut_attenuation[chip->slots[slot1].ch1_level]) +
                                 ...
                                 (output4 * chip->lut_attenuation[chip->slots[slot4].ch1_level]))
                                        >> 16;
                    }
                }
                break;
                ...
                for (op = 0; op < 2; op++) {
                    ...
                    if (chip->slots[slot1].active) {
                        for (i = 0; i < samples; i++) {
                            ...
                            *mixp++ += ((output1 *
                                         chip->lut_attenuation[chip->slots[slot1].ch0_level]) +
                                        (output3 *
                                         chip->lut_attenuation[chip->slots[slot3].ch0_level]))
                                    >> 16;
                            *mixp++ += ((output1 *
                                         chip->lut_attenuation[chip->slots[slot1].ch1_level]) +
                                        (output3 *
                                         chip->lut_attenuation[chip->slots[slot3].ch1_level]))
                                    >> 16;
                        }
                    }
                }
...
static void init_tables(YMF271Chip *chip) {
    ...
    for (i = 0; i < SIN_LEN; i++) {
        ...
        chip->lut_waves[1][i] = (i < (SIN_LEN / 2)) ? (INT16) ((m * m) * MAXOUT) : (INT16) (
                (m * m) * MINOUT);
