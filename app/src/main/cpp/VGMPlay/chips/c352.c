...
void c352_update(UINT8 ChipID, stream_sample_t **outputs, int samples) {
    ...
    for (i = 0; i < samples; i++) {
        for (j = 0; j < 32; j++) {
            ...
            if (!c->v[j].mute) {
                // Left
                outputs[0][i] += (c->v[j].flags & C352_FLG_PHASEFL) ? (-s * (c->v[j].vol_f >> 8))
                        >> 8
                                                                    : (s * (c->v[j].vol_f >> 8))
                                         >> 8;
                if (!c->muteRear && !MuteAllRear)
                    outputs[0][i] += (c->v[j].flags & C352_FLG_PHASERL) ?
                                     (-s * (c->v[j].vol_r >> 8)) >> 8
                                                                        : (s * (c->v[j].vol_r >> 8))
                                             >> 8;

                // Right
                outputs[1][i] += (c->v[j].flags & C352_FLG_PHASEFR) ? (-s * (c->v[j].vol_f & 0xff))
                        >> 8
                                                                    : (s * (c->v[j].vol_f & 0xff))
                                         >> 8;
                if (!c->muteRear && !MuteAllRear)
