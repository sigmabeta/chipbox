...
UINT8 k053260_r(UINT8 ChipID, offs_t offset) {
    ...
    switch (offset) {
        ...
        case 0x2e: /* read ROM */
            if (ic->mode & 1) {
                UINT32 offs = ic->channels[0].start + (ic->channels[0].pos >> BASE_SHIFT) +
                              (ic->channels[0].bank << 16);
