...
//	case 0x15:	// MultiPCM
        ...
        case 0x1B:    // HuC6280
            ...
            if (Port == 0xFF) {
                ...
                if (chip->DstChipType == 0x05);    // TODO
                else if (chip->DstChipType == 0x05);    // TODO
        ...
        case 0x1F:    // QSound
            Command = (chip->DstCommand & 0x00FF) >> 0;
            chip_reg_write(chip->DstChipType, chip->DstChipID, ChipData[0x00], ChipData[0x01],
                           Command);
            break;
    }
    chip->Running |= 0x10;
...
void
daccontrol_set_data(UINT8 ChipID, UINT8 *Data, UINT32 DataLen, UINT8 StepSize, UINT8 StepBase) {
    dac_control *chip = &DACData[ChipID];
