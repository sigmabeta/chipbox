...
//		gb->snd_2.env_direction = (data & 0x8 ) >> 3;
        ...
        case NR23: /* Frequency lo (R/W) */
            gb->snd_2.period = gb->period_table[((gb->snd_regs[NR24] & 0x7) << 8) |
                                                gb->snd_regs[NR23]];
            break;
        ...
        case NR33: /* Frequency lo (W) */
            gb->snd_3.period = gb->period_mode3_table[((gb->snd_regs[NR34] & 0x7) << 8) +
                                                      gb->snd_regs[NR33]];
            break;
...
void gameboy_update(UINT8 ChipID, stream_sample_t **outputs, int samples) {
    ...
    while (samples-- > 0) {
        ...
        if (gb->snd_1.on && !gb->snd_1.Muted) {
            ...
            if (!AccuracyHack) {
                gb->snd_1.pos++;
                if (gb->snd_1.pos ==
                    (UINT32) (gb->snd_1.period / wave_duty_table[gb->snd_1.duty]) >> FIXED_POINT) {
        ...
        if (gb->snd_2.on && !gb->snd_2.Muted) {
            ...
            if (!AccuracyHack) {
                gb->snd_2.pos++;
                if (gb->snd_2.pos ==
                    (UINT32) (gb->snd_2.period / wave_duty_table[gb->snd_2.duty]) >> FIXED_POINT) {
        ...
        if (gb->snd_3.on && !gb->snd_3.Muted) {
            ...
            if (!AccuracyHack) {
                ...
                if (gb->snd_3.pos >= ((UINT32) (((gb->snd_3.period) >> 21)) + gb->snd_3.duty)) {
                    gb->snd_3.pos = 0;
                    if (gb->snd_3.dutycount ==
                        ((UINT32) (((gb->snd_3.period) >> FIXED_POINT)) % 32)) {
                        gb->snd_3.duty--;
                    }
                    gb->snd_3.dutycount++;
                    gb->snd_3.offset++;
        ...
        if (gb->snd_4.on && !gb->snd_4.Muted) {
            ...
            if (gb->snd_4.pos == (gb->snd_4.period >> (FIXED_POINT + 1))) {
                ...
                mode4_mask = (((gb->snd_4.ply_value & 0x2) >> 1) ^ (gb->snd_4.ply_value & 0x1))
                        << (gb->snd_4.ply_step ? 6 : 14);
    ...
    gb->snd_regs[NR52] =
            (gb->snd_regs[NR52] & 0xf0) | gb->snd_1.on | (gb->snd_2.on << 1) | (gb->snd_3.on << 2) |
            (gb->snd_4.on << 3);
}
...
int device_start_gameboy_sound(UINT8 ChipID, int clock) {
    ...
    for (I = 0; I < 8; I++) {
        for (J = 0; J < 16; J++) {
            ...
            gb->period_mode4_table[I][J] =
                    ((1 << FIXED_POINT) / (524288 / ((I == 0) ? 0.5 : I) / (1 << (J + 1)))) *
                    gb->rate;
        }
    }
    ...
    for (I = 0; I < 256; I++) {
        gb->length_mode3_table[I] =
                ((256 - I) * ((1 << FIXED_POINT) / 256) * gb->rate) >> FIXED_POINT;
    }
