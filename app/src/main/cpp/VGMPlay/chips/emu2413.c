...
makeAdjustTable(void) {
    ...
    for (i = 1; i < (1 << EG_BITS); i++)
        AR_ADJUST_TABLE[i] = (e_uint16) ((double) (1 << EG_BITS) - 1 -
...
makeDB2LinTable(void) {
    ...
    for (i = 0; i < DB_MUTE + DB_MUTE; i++) {
        DB2LIN_TABLE[i] = (e_int16) ((double) ((1 << DB2LIN_AMP_BITS) - 1) *
...
makePmTable(void) {
    ...
    for (i = 0; i < PM_PG_WIDTH; i++)
        ...
        pmtable[i] = (e_int32) ((double) PM_AMP *
...
makeAmTable(void) {
    ...
    for (i = 0; i < AM_PG_WIDTH; i++)
        ...
        amtable[i] = (e_int32) ((double) AM_DEPTH / 2 / DB_STEP *
...
makeDphaseTable(void) {
    ...
    e_uint32 mltable[16] =
            {1, 1 * 2, 2 * 2, 3 * 2, 4 * 2, 5 * 2, 6 * 2, 7 * 2, 8 * 2, 9 * 2, 10 * 2, 10 * 2,
             12 * 2, 12 * 2, 15 * 2, 15 * 2};

    for (fnum = 0; fnum < 512; fnum++)
        for (block = 0; block < 8; block++)
            for (ML = 0; ML < 16; ML++)
                dphaseTable[fnum][block][ML] = RATE_ADJUST (
...
#define dB2(x) ((x)*2)
    ...
    static double kltable[16] = {
            ...
            dB2 (16.875), dB2 (17.625),
            dB2 (18.000), dB2 (18.750), dB2 (19.125), dB2 (19.500), dB2 (19.875), dB2 (20.250),
            dB2 (20.625), dB2 (21.000)
    };
                    ...
                    if (KL == 0) {
                        ...
                        if (tmp <= 0)
                            ...
                            tllTable[fnum][block][TL][KL] =
                                    (e_uint32) ((tmp >> (3 - KL)) / EG_STEP) + TL2EG (TL);
                    }
                }
}
...
OPLL_patch2dump(const OPLL_PATCH *patch, e_uint8 *dump) {
    dump[0] = (e_uint8) ((patch[0].AM << 7) + (patch[0].PM << 6) + (patch[0].EG << 5) +
                         (patch[0].KR << 4) + patch[0].ML);
    dump[1] = (e_uint8) ((patch[1].AM << 7) + (patch[1].PM << 6) + (patch[1].EG << 5) +
                         (patch[1].KR << 4) + patch[1].ML);
    dump[2] = (e_uint8) ((patch[0].KL << 6) + patch[0].TL);
    dump[3] = (e_uint8) ((patch[1].KL << 6) + (patch[1].WF << 4) + (patch[0].WF << 3) +
                         patch[0].FB);
...
slotOff(OPLL_SLOT *slot) {
    if (slot->eg_mode == ATTACK)
        slot->eg_phase = EXPAND_BITS (
                AR_ADJUST_TABLE[HIGHBITS(slot->eg_phase, EG_DP_BITS - EG_BITS)], EG_BITS,
                EG_DP_BITS);
...
#define S2E(x) (SL2EG((e_int32)(x/SL_STEP))<<(EG_DP_BITS-EG_BITS))
    ...
    static e_uint32 SL[16] = {
            ...
            S2E (21.0),
            S2E (24.0), S2E (27.0), S2E (30.0), S2E (33.0), S2E (36.0), S2E (39.0), S2E (42.0),
            S2E (48.0)
    };
...
calc_slot_car(OPLL_SLOT *slot, e_int32 fm) {
    if (slot->egout >= (DB_MUTE - 1)) {
        ...
        slot->output[0] = DB2LIN_TABLE[
                slot->sintbl[(slot->pgout + wave2_8pi(fm)) & (PG_WIDTH - 1)] + slot->egout];
    }
...
//  e_int32 r[4] = { 0, 0, 0, 0 };        /* Ignore, Right, Left, Center */
    ...
    if (!opll->vrc7_mode) {
        ...
        if (opll->patch_number[7] <= 15) {
            ...
            if (!(opll->mask & OPLL_MASK_SD) && (CAR (opll, 7)->eg_mode != FINISH)) {
                channel = -(calc_slot_snare(CAR (opll, 7), opll->noise_seed & 1)
                        << 1); // this one is negated
