...
static int
generate_adpcm(struct YMZ280BVoice *voice, UINT8 *base, UINT32 size, INT16 *buffer, int samples) {
...
static int
generate_pcm8(struct YMZ280BVoice *voice, UINT8 *base, UINT32 size, INT16 *buffer, int samples) {
    int position = voice->position;
    int val;
...
static int
generate_pcm16(struct YMZ280BVoice *voice, UINT8 *base, UINT32 size, INT16 *buffer, int samples) {
    int position = voice->position;
    int val;
    ...
    if (!voice->looping) {
        ...
        while (samples) {
            ...
            val = (INT16) ((ymz280b_read_memory(base, size, position / 2 + 0) << 8) +
                           ymz280b_read_memory(base, size, position / 2 + 1));
...
void ymz280b_update(UINT8 ChipID, stream_sample_t **outputs, int samples) {
    ...
    for (v = 0; v < 8; v++) {
        ...
        while (remaining > 0 && voice->output_pos < FRAC_ONE) {
            int interp_sample = (((INT32) prev * (FRAC_ONE - voice->output_pos)) +
                                 ((INT32) curr * voice->output_pos)) >> FRAC_BITS;
        ...
        switch (voice->playing << 7 | voice->mode) {
            case 0x81:
                samples_left = generate_adpcm(voice, chip->region_base, chip->region_size,
                                              chip->scratch, new_samples);
                break;
            case 0x82:
                samples_left = generate_pcm8(voice, chip->region_base, chip->region_size,
                                             chip->scratch, new_samples);
                break;
            case 0x83:
                samples_left = generate_pcm16(voice, chip->region_base, chip->region_size,
                                              chip->scratch, new_samples);
                break;
            default:
                samples_left = 0;
        ...
        switch (chip->current_register) {
            ...
            case 0x86:      /* ROM readback / RAM write (low) -> update latch */
                ...
                if (chip->ext_mem_enable)
                    chip->ext_readlatch = ymz280b_read_memory(chip->region_base, chip->region_size,
