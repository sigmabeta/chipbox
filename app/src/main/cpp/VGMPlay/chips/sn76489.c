...
void
SN76489_Config(SN76489_Context *chip, /*int mute,*/ int feedback, int sr_width, int boost_noise) {
    //chip->Mute = mute;
...
void SN76489_Write(SN76489_Context *chip, int data) {
    ...
    switch (chip->LatchedRegister) {
        ...
        case 6: /* Noise */
            ...
            chip->NoiseFreq =
                    0x10 << (chip->Registers[6] & 0x3); /* set noise signal generator frequency */
            break;
    }
}
...
void SN76489_Update(SN76489_Context *chip, INT32 **buffer, int length) {
    ...
    for (j = 0; j < length; j++) {
        ...
        for (i = 0; i <= 2; ++i)
            if ((chip_t->Mute >> i) & 1) {
                if (chip_t->IntermediatePos[i] != FLT_MIN)
                    ...
                    chip->Channels[i] = (short) (PSGVolumeValues[chip->Registers[2 * i + 1]] *
                                                 chip_t->IntermediatePos[i]);
                else
                    /* Flat (no antialiasing needed) */
                    chip->Channels[i] =
                            PSGVolumeValues[chip->Registers[2 * i + 1]] * chip_t->ToneFreqPos[i];
            } else
                /* Muted channel */
                chip->Channels[i] = 0;
        ...
        if ((chip_t->Mute >> 3) & 1) {
            ...
            chip->Channels[3] = PSGVolumeValues[chip->Registers[7]] *
                                ((chip_n->NoiseShiftRegister & 0x1) * 2 - 1);
        ...
        for (i = 0; i <= 2; ++i) {
            if (chip->ToneFreqVals[i] <= 0) {   /* If the counter gets below 0... */
                if (chip->Registers[i * 2] >= PSG_CUTOFF) {
                    ...
                    chip->IntermediatePos[i] =
                            (chip->NumClocksForSample - chip->Clock + 2 * chip->ToneFreqVals[i]) *
                            chip->ToneFreqPos[i] / (chip->NumClocksForSample + chip->Clock);
                    /* Flip the flip-flop */
                ...
                chip->ToneFreqVals[i] += chip->Registers[i * 2] *
                                         (chip->NumClocksForSample / chip->Registers[i * 2] + 1);
            } else
        ...
        if (chip->ToneFreqVals[3] <= 0) {
            ...
            if (chip->NoiseFreq != 0x80)
                ...
                chip->ToneFreqVals[3] +=
                        chip->NoiseFreq * (chip->NumClocksForSample / chip->NoiseFreq + 1);
            if (chip->ToneFreqPos[3] == 1) {
                    ...
                    switch (chip->WhiteNoiseFeedback) {
                        ...
                        case 0x0009: /* SMS, GG, MD  %00001001 */
                            ...
                            Feedback = ((chip->NoiseShiftRegister & chip->WhiteNoiseFeedback)
                                        && ((chip->NoiseShiftRegister & chip->WhiteNoiseFeedback) ^
                ...
                chip->NoiseShiftRegister =
                        (chip->NoiseShiftRegister >> 1) | (Feedback << (chip->SRWidth - 1));
            }
        }
    }
}
