...
static const Bit8u regbase2modop[44] = {
        0, 1, 2, 0, 1, 2, 0, 0, 3, 4, 5, 3, 4, 5, 0, 0, 6, 7, 8, 6, 7,
        8,                    // first set
        18, 19, 20, 18, 19, 20, 0, 0, 21, 22, 23, 21, 22, 23, 0, 0, 24, 25, 26, 24, 25,
        26    // second set
};
static const Bit8u regbase2op[44] = {
        0, 1, 2, 9, 10, 11, 0, 0, 3, 4, 5, 12, 13, 14, 0, 0, 6, 7, 8, 15, 16,
        17,            // first set
        18, 19, 20, 27, 28, 29, 0, 0, 21, 22, 23, 30, 31, 32, 0, 0, 24, 25, 26, 33, 34,
        35    // second set
};
...
INLINE void
operator_advance_drums(OPL_DATA *chip, op_type *op_pt1, Bit32s vib1, op_type *op_pt2, Bit32s vib2,
                       op_type *op_pt3, Bit32s vib3) {
    Bit32u c1 = op_pt1->tcount / FIXEDPT;
    Bit32u c3 = op_pt3->tcount / FIXEDPT;
    Bit32u phasebit = (((c1 & 0x88) ^ ((c1 << 5) & 0x80)) | ((c3 ^ (c3 << 2)) & 0x20)) ? 0x02
                                                                                       : 0x00;
...
INLINE void operator_output(op_type *op_pt, Bit32s modulator, Bit32s trem) {
    if (op_pt->op_state != OF_TYPE_OFF) {
        ...
        op_pt->cval = (Bit32s) (op_pt->step_amp * op_pt->vol *
                                op_pt->cur_wform[i & op_pt->cur_wmask] * trem / 16.0);
    }
}
...
static void operator_attack(op_type *op_pt) {
    ...
    for (ct = 0; ct < num_steps_add; ct++) {
        ...
        if ((op_pt->cur_env_step & op_pt->env_step_a) ==
            ...
            if (op_pt->step_skip_pos_a &
                op_pt->env_step_skip_a) {    // check if required to skip next step
                op_pt->step_amp = op_pt->amp;
            }
        }
    }
...
static void change_attackrate(OPL_DATA *chip, Bitu regbase, op_type *op_pt) {
    ...
    if (attackrate) {
        ...
        fltype f = (fltype) (pow(FL2, (fltype) attackrate + (op_pt->toff >> 2) - 1) *
                             attackconst[op_pt->toff & 3] * chip->recipsamp);
...
static void change_decayrate(OPL_DATA *chip, Bitu regbase, op_type *op_pt) {
    ...
    if (decayrate) {
        ...
        op_pt->decaymul = (fltype) (pow(FL2,
                                        f * pow(FL2, (fltype) (decayrate + (op_pt->toff >> 2)))));
...
static void change_releaserate(OPL_DATA *chip, Bitu regbase, op_type *op_pt) {
    ...
    if (releaserate) {
        ...
        fltype f = (fltype) (-7.4493 * decrelconst[op_pt->toff & 3] * chip->recipsamp);
        op_pt->releasemul = (fltype) (pow(FL2, f * pow(FL2, (fltype) (releaserate +
                                                                      (op_pt->toff >> 2)))));
...
static void change_frequency(OPL_DATA *chip, Bitu chanbase, Bitu regbase, op_type *op_pt) {
    ...
    // frequency
    frn = ((((Bit32u) chip->adlibreg[ARC_KON_BNUM + chanbase]) & 3) << 8) +
          (Bit32u) chip->adlibreg[ARC_FREQ_NUM + chanbase];
    // block number/octave
    ...
    // 20+a0+b0:
    op_pt->tinc = (Bit32u) ((((fltype) (frn << oct)) *
                             chip->frqmul[chip->adlibreg[ARC_TVS_KSR_MUL + regbase] & 15]));
    // 40+a0+b0:
    vol_in = (fltype) ((fltype) (chip->adlibreg[ARC_KSL_OUTLEV + regbase] & 63) +
                       kslmul[chip->adlibreg[ARC_KSL_OUTLEV + regbase] >> 6] *
                       kslev[oct][frn >> 6]);
...
void *adlib_OPL3_init(UINT32 clock, UINT32 samplerate,
    ...
    for (i = 15; i >= 0; i--) {
        OPL->frqmul[i] = (fltype) (frqmul_tab[i] * INTFREQU / (fltype) WAVEPREC * (fltype) FIXEDPT *
                                   OPL->recipsamp);
    }
    ...
    if (!initfirstime) {
        ...
        for (i = 0; i < TREMTAB_SIZE; i++) {
            ...
            fltype trem_val1 = (fltype) (((fltype) trem_table_int[i]) * 4.8 / 26.0 /
                                         6.0);                // 4.8db
            fltype trem_val2 = (fltype) ((fltype) ((Bit32s) (trem_table_int[i] / 4)) * 1.2 / 6.0 /
                                         6.0);        // 1.2db (larger stepping)
    ...
    // tremolo at 3.7hz
    OPL->tremtab_add = (Bit32u) ((fltype) TREMTAB_SIZE * TREM_FREQ * FIXEDPT_LFO /
                                 (fltype) OPL->int_samplerate);
    OPL->tremtab_pos = 0;
    ...
    if (!initfirstime) {
        ...
        for (i = 0; i < (WAVEPREC >> 1); i++) {
            wavtable[(i << 1) + WAVEPREC] = (Bit16s) (16384 *
                                                      sin((fltype) ((i << 1)) * PI * 2 / WAVEPREC));
            wavtable[(i << 1) + 1 + WAVEPREC] = (Bit16s) (16384 *
                                                          sin((fltype) ((i << 1) + 1) * PI * 2 /
                                                              WAVEPREC));
...
static void adlib_write(void *chip, Bitu idx, Bit8u val) {
    ...
    switch (idx & 0xf0) {
        ...
        case ARC_KON_BNUM: {
            ...
            if (base < 9) {
                ...
                if (val & 32) {
                    ...
                    enable_operator(chip, modbase, &OPL->op[opbase],
                                    OP_ACT_NORMAL);        // modulator (if 2op)
                    enable_operator(chip, modbase + 3, &OPL->op[opbase + 9],
                                    OP_ACT_NORMAL);    // carrier (if 2op)
#if defined(OPLTYPE_IS_OPL3)
                    ...
                    if ((OPL->adlibreg[0x105] & 1) && OPL->op[opbase].is_4op) {
                        ...
                        enable_operator(chip, modbase + 3 + 8, &OPL->op[opbase + 3 + 9],
                                        OP_ACT_NORMAL);
                    }
#endif
                } else {
        ...
        case ARC_WAVE_SEL + 0x10: {
            ...
            if ((num < 6) && (base < 22)) {
#if defined(OPLTYPE_IS_OPL3)
                Bits wselbase = second_set ? (base + 22)
                                           : base;    // for easier mapping onto wave_sel[]
                op_type *op_ptr;
                // change waveform
                if (OPL->adlibreg[0x105] & 1)
                    OPL->wave_sel[wselbase] = val &
...
Bitu adlib_OPL3_reg_read(void *chip, UINT32 port) {
    OPL_DATA *OPL = (OPL_DATA *) chip;
...
void adlib_OPL3_write_index(void *chip, UINT32 port, UINT8 val) {
    ...
    if ((port & 3) != 0) {
        // possibly second set
        if (((OPL->adlibreg[0x105] & 1) != 0) || (OPL->opl_index == 5))
            OPL->opl_index |= ARC_SECONDSET;
    }
#endif
}
...
//#endif
        ...
        vib_tshift = ((OPL->adlibreg[ARC_PERC_MODE] & 0x40) == 0) ? 1
                                                                  : 0;    // 14cents/7cents switching
        for (i = 0; i < endsamples; i++) {
            ...
            vib_lut[i] = vib_table[OPL->vibtab_pos / FIXEDPT_LFO]
                    >> vib_tshift;        // 14cents (14/100 of a semitone) or 7cents
        ...
        if (OPL->adlibreg[ARC_PERC_MODE] & 0x20) {
                ...
                if (OPL->adlibreg[ARC_FEEDBACK + 6] & 1) {
                    ...
                    if (cptr[9].op_state != OF_TYPE_OFF) {
                        if (cptr[9].vibrato) {
                            vibval1 = vibval_var1;
                            for (i = 0; i < endsamples; i++)
                                vibval1[i] = (Bit32s) ((vib_lut[i] * cptr[9].freq_high / 8) *
                                                       FIXEDPT * VIBFAC);
                        } else
                    ...
                    if ((cptr[9].op_state != OF_TYPE_OFF) || (cptr[0].op_state != OF_TYPE_OFF)) {
                        if ((cptr[0].vibrato) && (cptr[0].op_state != OF_TYPE_OFF)) {
                            vibval1 = vibval_var1;
                            for (i = 0; i < endsamples; i++)
                                vibval1[i] = (Bit32s) ((vib_lut[i] * cptr[0].freq_high / 8) *
                                                       FIXEDPT * VIBFAC);
                        } else
                            vibval1 = vibval_const;
                        if ((cptr[9].vibrato) && (cptr[9].op_state != OF_TYPE_OFF)) {
                            vibval2 = vibval_var2;
                            for (i = 0; i < endsamples; i++)
                                vibval2[i] = (Bit32s) ((vib_lut[i] * cptr[9].freq_high / 8) *
                                                       FIXEDPT * VIBFAC);
                        } else
                        ...
                        for (i = 0; i < endsamples; i++) {
                            ...
                            operator_output(&cptr[0],
                                            (cptr[0].lastcval + cptr[0].cval) * cptr[0].mfbi / 2,
                                            tremval1[i]);
            ...
            if (!(OPL->MuteChn[NUM_CHANNELS + 2]) && OPL->op[8].op_state != OF_TYPE_OFF) {
                ...
                if (cptr[0].vibrato) {
                    vibval3 = vibval_var1;
                    for (i = 0; i < endsamples; i++)
                        vibval3[i] = (Bit32s) ((vib_lut[i] * cptr[0].freq_high / 8) * FIXEDPT *
                                               VIBFAC);
                } else
            ...
            if ((OPL->op[7].op_state != OF_TYPE_OFF) || (OPL->op[16].op_state != OF_TYPE_OFF) ||
                ...
                if ((cptr[0].vibrato) && (cptr[0].op_state != OF_TYPE_OFF)) {
                    vibval1 = vibval_var1;
                    for (i = 0; i < endsamples; i++)
                        vibval1[i] = (Bit32s) ((vib_lut[i] * cptr[0].freq_high / 8) * FIXEDPT *
                                               VIBFAC);
                } else
                    vibval1 = vibval_const;
                if ((cptr[9].vibrato) && (cptr[9].op_state == OF_TYPE_OFF)) {
                    vibval2 = vibval_var2;
                    for (i = 0; i < endsamples; i++)
                        vibval2[i] = (Bit32s) ((vib_lut[i] * cptr[9].freq_high / 8) * FIXEDPT *
                                               VIBFAC);
                } else
                    vibval2 = vibval_const;
                ...
                if ((cptr[9].vibrato) && (cptr[9].op_state == OF_TYPE_OFF)) {
                    vibval4 = vibval_var2;
                    for (i = 0; i < endsamples; i++)
                        vibval4[i] = (Bit32s) ((vib_lut[i] * cptr[9].freq_high / 8) * FIXEDPT *
                                               VIBFAC);
                } else
                ...
                for (i = 0; i < endsamples; i++) {
                    ...
                    operator_advance_drums(OPL, &OPL->op[7], vibval1[i], &OPL->op[7 + 9],
                                           vibval2[i], &OPL->op[8 + 9], vibval4[i]);
        ...
        for (cur_ch = max_channel - 1; cur_ch >= 0; cur_ch--) {
            ...
            if (OPL->adlibreg[ARC_FEEDBACK + k] & 1) {
                    ...
                    if (OPL->adlibreg[ARC_FEEDBACK + k + 3] & 1) {
                        ...
                        if (cptr[0].op_state != OF_TYPE_OFF) {
                            ...
                            for (i = 0; i < endsamples; i++) {
                                ...
                                opfuncs[cptr[0].op_state](&cptr[0]);
                                operator_output(&cptr[0],
                                                (cptr[0].lastcval + cptr[0].cval) * cptr[0].mfbi /
                                                2, tremval1[i]);

                                chanval = cptr[0].cval;
                                CHANVAL_OUT(
                                        3)    // Note: Op 1 of 4, so it needs to use the panning bits of Op 4 (Ch+3)
                            }
                        }

                        if ((cptr[3].op_state != OF_TYPE_OFF) ||
                ...
                for (i = 0; i < endsamples; i++) {
                    ...
                    operator_advance(OPL, &cptr[0], vibval1[i]);
                    opfuncs[cptr[0].op_state](&cptr[0]);
                    operator_output(&cptr[0], (cptr[0].lastcval + cptr[0].cval) * cptr[0].mfbi / 2,
                                    tremval1[i]);
