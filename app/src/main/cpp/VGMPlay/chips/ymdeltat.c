    ...
    if ((DELTAT->portstate & 0xe0) == 0x20) {
        ...
        if (DELTAT->now_addr != (DELTAT->end << 1)) {
            ...
            if (DELTAT->status_reset_handler)
                if (DELTAT->status_change_BRDY_bit)
                    (DELTAT->status_reset_handler)(DELTAT->status_change_which_chip,
                                                   DELTAT->status_change_BRDY_bit);
            ...
            */
            ...
            if (DELTAT->status_set_handler)
                if (DELTAT->status_change_BRDY_bit)
                    (DELTAT->status_set_handler)(DELTAT->status_change_which_chip,
                                                 DELTAT->status_change_BRDY_bit);
        } else {
...
*/
            ...
            DELTAT->portstate = v & (0x80 | 0x40 | 0x20 | 0x10 |
                                     0x01); /* start, rec, memory mode, repeat flag copy, reset(bit0) */
...
*/
        ...
        case 0x0a:    /* DELTA-N H */
            ...
            DELTAT->step = (UINT32) ((double) (DELTAT->delta /* *(1<<(YM_DELTAT_SHIFT-16)) */ ) *
                                     (DELTAT->freqbase));
...
*/
            ...
            if (oldvol != 0) {
                DELTAT->adpcml = (int) ((double) DELTAT->adpcml / (double) oldvol *
...
void YM_DELTAT_ADPCM_Reset(YM_DELTAT *DELTAT, int pan, int emulation_mode) {
    ...
    DELTAT->control2 = (emulation_mode == YM_DELTAT_EMULATION_MODE_YM2610) ? 0x01
                                                                           : 0;    /* default setting depends on the emulation mode. MSX demo called "facdemo_4" doesn't setup control2 register at all and still works */
...
*/
    ...
    if ((DELTAT->portstate & 0xe0) == 0x80) {
        ...
        YM_DELTAT_synthesis_from_CPU_memory(
                DELTAT);    /* change output based on data in ADPCM data reg ($08) */
        return;
    }
