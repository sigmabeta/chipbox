...
class BrowseByGameScreenshots(
    ...
    @Test
    fun browseByGameScreen() {
        paparazzi.unsafeUpdateConfig(deviceConfig = deviceConfig)
        paparazzi.snapshot {
            BrowseByGame(syntheticWidthClass = deviceConfig.toWidthClass())
        }
    }

    @Test
    fun browseByGameScreenLoading() {
        paparazzi.unsafeUpdateConfig(deviceConfig = deviceConfig)
        paparazzi.snapshot {
            BrowseByGameLoading(syntheticWidthClass = deviceConfig.toWidthClass())
        }
    }
