...
class BrowseByArtistViewModel @Inject constructor(
    ...
    private fun loadNextPage() {
        ...
        val loaded = (current.artists as? LCE.Content)?.data ?: return
    ...
    private fun loadPreviousPage() {
        if (pageJob?.isActive == true) return
        val current = state.value
        if (current.artists !is LCE.Content || !current.hasMoreBefore) return
    ...
    private suspend fun fetchPage(offset: Int, limit: Int): Data<List<Artist>> = repository
        .getAllArtists(limit = limit, offset = offset)
        .first { it !is Data.Loading }

    private fun reduceInitial(state: BrowseByArtistState, data: Data<List<Artist>>) = when (data) {
        is Data.Succeeded -> state.copy(
            artists = LCE.Content(data.data),
            loadingPrevious = false,
            loadingMore = false,
        ...
        is Data.Failed -> state.copy(artists = LCE.Error(LOAD_OP, IllegalStateException(data.message)))

        Data.Loading -> state
    }

    private fun reduceAppend(state: BrowseByArtistState, data: Data<List<Artist>>) = when (data) {
        is Data.Succeeded -> {
            val current = (state.artists as? LCE.Content)?.data.orEmpty()
            state.copy(
                artists = LCE.Content(current + data.data),
                loadingMore = false,
                hasMoreAfter = data.data.size >= pageSize,
            )
        }
    ...
    private fun reducePrepend(state: BrowseByArtistState, data: Data<List<Artist>>, newStart: Int) = when (data) {
        is Data.Succeeded -> {
            val current = (state.artists as? LCE.Content)?.data.orEmpty()
            state.copy(
                artists = LCE.Content(data.data + current),
